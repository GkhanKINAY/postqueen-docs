...
<Tabs>
  <Tab title="PostQueen Cloud">
    ...
    **Pros:**
    ...
    see the [status of every network](/providers/overview#status-on-postqueen).
    </Note>
  </Tab>
...
<AccordionGroup>
  <Accordion title="The network I want is not in Add Channel" icon="plug-circle-xmark">
    Every network is listed in **Add Channel** on PostQueen, but some are marked **Soon**: they cannot be connected yet. The [status of every network](/providers/overview#status-on-postqueen) says which. On a self-hosted install a network only appears once its OAuth app is registered and the instance has been restarted. [Providers](/providers/overview) has the Cloud connect flow; [self-hosting provider pages](/installation/providers/instagram) have the OAuth app setup.
  </Accordion>
