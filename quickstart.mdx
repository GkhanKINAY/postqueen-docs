...
<AccordionGroup>
  <Accordion title="The network I want is not in Add Channel" icon="plug-circle-xmark">
    On PostQueen Cloud every supported network is already in **Add Channel**. On a self-hosted install a network only appears once its OAuth app is registered and she has been restarted. [Providers](/providers/overview) has the Cloud connect flow; [self-hosting provider pages](/installation/providers/instagram) have the OAuth app setup.
  </Accordion>
