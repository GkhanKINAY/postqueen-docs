...
{/* OVERHAUL-TODO(B1): write it from _templates/guide-task.mdx. Sources: this file (rewrite; WIP: better, use as base; audit: guide). Row in page-status-2026-09-23.md. */}
...
<Tabs>
  <Tab title="PostQueen Cloud">
    Sign up at [app.postqueen.ai](https://app.postqueen.ai). Nothing to install and nothing to keep
    patched.

    **Best for:** getting a post out today, and anyone who does not want to run a server.

    **Pros:**
    - A 7-day trial with $0 due today. A card is required: PostQueen places a $1 authorization on it
      and releases it at once
    - Upgrades and security patches happen without you

    <Note>
    Two things wait for the paid plan: connecting X, and the AI Copilot. They unlock when the trial
    ends, or right away if you end the trial early. [Free trial](/cloud/trial) has the details.

    Not every network can be connected yet. Some are marked **Soon** or **In review** on PostQueen:
    see the [status of every network](/channels/overview#status-on-postqueen).
    </Note>
  </Tab>
  ...
  <Tab title="Your own server">
    ...
    **Pros:**
    ...
    [Providers](/channels/overview) walks through each. Plan for it before you promise a client a
    channel.
    </Warning>
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Pick the network">
    ...
    - **A smaller group asks for a credential in the form.** Bluesky takes an app password, and
      WordPress takes a site URL and an application password.

    [Providers](/channels/overview) says which kind each network is, and what it wants.
  </Step>
...
<AccordionGroup>
  <Accordion title="The network I want is not in Add Channel" icon="unplug">
    Every network is listed in **Add Channel** on PostQueen, but some are marked **Soon**: they cannot be connected yet. The [status of every network](/channels/overview#status-on-postqueen) says which. On a self-hosted install a network only appears once its OAuth app is registered and the instance has been restarted. [Providers](/channels/overview) has the Cloud connect flow; [self-hosting provider pages](/installation/providers/instagram) have the OAuth app setup.
  </Accordion>
  ...
  <Accordion title="The save was refused and named a channel" icon="triangle-alert">
  ...
  <Accordion title="Reveal shows nothing under API Keys" icon="key">
    **Connections > API Keys** is visible to every member, but only a workspace admin can reveal or
    rotate the key; members see it masked. Calling the public API also needs a subscription, and the
    free trial counts.
    Self-hosted installs with billing off have it on by default.
  </Accordion>

  <Accordion title="My agent connected but cannot see any channels" icon="bot">
    ...
    installs use. [CLI authentication](/cli/introduction) has the exact shape.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="The composer in full" icon="pen" href="/using/create-a-post">
  ...
  <Card title="Connect more channels" icon="plug" href="/channels/overview">
    Every network she posts to, and the OAuth app each one wants.
  </Card>
  <Card title="Run her from anywhere" icon="terminal" href="/introduction">
