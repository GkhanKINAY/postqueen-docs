...
<AccordionGroup>
  <Accordion title="pnpm runs out of heap during install" icon="memory-stick">
    ...
    [System requirements](/installation/overview) has the minimums.
    </Warning>
  </Accordion>
  ...
  <Accordion title="postgres ECONNREFUSED" icon="database">
    ...
    |---|---|
    | `DATABASE_URL` host | `localhost` fails from a container without host networking. Use the service name, `postqueen-postgres` |
    | Postgres is up | `docker compose logs postqueen-postgres` shows *database system is ready to accept connections* |
  ...
  <Accordion title="Default ports, and which one to open" icon="plug">
    ...
    | From source, `pnpm dev` | Frontend `4200`, backend `3000` (override with `PORT`). Put a [reverse proxy](/installation/domain-and-https) in front for one URL |

    Temporal listens on `7233`, and its UI on `8080`, either way.
  </Accordion>

  <Accordion title="Email is not sending" icon="mail">
  ...
  <Accordion title="Can I deploy on Vercel?" icon="triangle-alert">
  ...
  <Accordion title="Hiding a provider from the interface" icon="eye-off">
