...
| "Too many attempts, request a new code" | A code was typed wrong five times. Ask for a new one. |
...
| "You have reached the maximum number of channels for your subscription. Please upgrade your subscription to add more channels." | Disabled channels count toward the limit. Delete a channel you no longer need, or choose **Move to billing** and pick a bigger plan: [Plans and limits](/cloud/plans). |
| "Could not connect to the platform" | Try again. If it keeps failing, [contact support](/support). |
| **Enable Channel** says "Channel Enabled", but the channel stays off | Your enabled channels are at the plan's limit. Disable another channel first: [Known issues](/troubleshooting/known-issues). |
...
| A file type "is not allowed" | Upload JPEG, PNG, GIF, WebP, AVIF, BMP, TIFF, MP4 or MOV. A MOV is converted to MP4 after upload. |
...
| "You are currently on trial, in order to use the feature you must finish the trial" | Choose **Finish the trial, charge me now**, or close the dialog and wait for the trial to end. |
