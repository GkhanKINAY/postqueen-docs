...
<AccordionGroup>
  ...
  <Accordion title="upload-from-url times out, or fetch failed" icon="link-slash">
    ...
    - A URL that resolves to a private or internal address is refused by design, with `Error: Blocked IP` in the logs. On a trusted private network, see [`DISABLE_SSRF_PROTECTION`](/configuration/reference#disable_ssrf_protection)
