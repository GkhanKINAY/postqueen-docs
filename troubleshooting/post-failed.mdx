...
{/* OVERHAUL-TODO(B10): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: 3 attempts \; audit: Two minutes apart ... 15 attempts" -> the publish call is never retried automatically (`maximumAttempts: 1`). The workflow mak… / [OTHER] "Most of the time that tooltip reads *An error occurred while publishing*" -> the tooltip shows the stored error, with that text only as a fallback (`launches/…). Row in page-status-2026-09-23.md. */}
...
[OAuth and channel connect errors](/troubleshooting/common-errors) has the message-by-message list.
...
<CardGroup cols={2}>
  <Card title="Your provider's page" icon="plug" href="/channels/overview">
    Platform-specific rejections, settings and limits, network by network
  </Card>
  <Card title="Reconnecting a channel" icon="link" href="/troubleshooting/common-errors">
