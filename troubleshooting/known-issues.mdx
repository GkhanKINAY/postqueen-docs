...
Bugs we have confirmed and are tracking. Each entry lists a workaround if
one exists. Once a fix ships, the entry will be removed (or moved into
release notes).

If you hit something that is not here, please report it on
...
still completes. Refresh the Channels page after authorizing on
Google's side.
...
PostQueen backend. Use externally-generated images and the
...
Deleting a post that has already been removed currently surfaces a 500
