## Controlling container services

When the environment variable `POSTQUEEN_APPS` is not set, or is set to an empty string, all
services will be started in a single container. This is normally fine for small, personal deployments.

However, you can only start specific services within the docker container by changing this environment variable.

If you need to scale, you can experiment with having multiple containers defined like;
- Frontend only: `POSTQUEEN_APPS="frontend"`
- Backend only: `POSTQUEEN_APPS="backend"`
- Worker and Cron only: `POSTQUEEN_APPS="worker cron"`

