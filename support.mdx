...
{/* OVERHAUL-TODO(B10): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: same as main; audit: cli-help). Row in page-status-2026-09-23.md. */}

Something is not working and you want a person to look at it. Here is where to go.

<CardGroup cols={2}>
  <Card title="GitHub Issues" icon="git-branch" href="https://github.com/GkhanKINAY/postqueen-app/issues/new/choose">
    Something is broken: a bug, an error, a crash. Screenshots and logs attached
  </Card>
  <Card title="Email" icon="mail" href="mailto:support@postqueen.ai">
...
<AccordionGroup>
  <Accordion title="Goal — what you wanted to happen" icon="target">
  ...
  <Accordion title="Environment — browser, OS, install" icon="monitor">
  ...
  <Accordion title="Changes — what moved before it broke" icon="rotate-ccw">
...
A charge is refunded in full if you ask within 30 days of it, on a monthly or a yearly plan. Say
which charge it is: [Refunds](/cloud/subscription#refunds).
...
window throwing localStorage away: [billing troubleshooting](/cloud/trial).
