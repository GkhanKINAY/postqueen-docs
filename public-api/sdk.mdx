...
{/* OVERHAUL-TODO(B8): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: partly better; audit: api). Row in page-status-2026-09-23.md. */}
...
The SDK authenticates with your Public API key, sent as the `Authorization` header on every request. Get it from **[app.postqueen.ai](https://app.postqueen.ai)** under **Connections > API Keys**, then **Reveal**. Only a workspace admin can reveal it.
...
<Info>
The create-post endpoint has **an hourly limit per workspace**. Batch multiple channels into a single `post()` call instead of one call per channel.
</Info>
