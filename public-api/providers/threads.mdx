---
title: Threads Settings
description: 'API settings for posting to Threads'
---

{/* OVERHAUL-TODO(B2): write it from _templates/api-settings.mdx. Sources: this file (fix; WIP: partly better; audit: api). Row in page-status-2026-09-23.md. */}

Creating the OAuth app and connecting a channel: [Threads setup guide](/installation/providers/threads).

## Settings Schema

When creating a post for Threads, use the following settings schema:

```json
{
  "settings": {
    "__type": "threads"
  }
}
```

<Note>
Threads has no provider-specific settings beyond `__type`. Post content,
images, and the schedule are configured at the post level - see the
[Create Post](/public-api/posts/create) reference.
</Note>

## Fields

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `__type` | `string` | Yes | Must be `threads` |

---

## Complete Example

### Text Post

```json
{
  "type": "schedule",
  "date": "2024-12-14T10:00:00.000Z",
  "shortLink": false,
  "tags": [],
  "posts": [
    {
      "integration": {
        "id": "your-threads-integration-id"
      },
      "value": [
        {
          "content": "Hello from the PostQueen API!",
          "image": []
        }
      ],
      "settings": {
        "__type": "threads"
      }
    }
  ]
}
```

### Thread Post

Create a thread by adding multiple items to the `value` array. Each entry
becomes a reply chained to the previous post:

```json
{
  "type": "schedule",
  "date": "2024-12-14T10:00:00.000Z",
  "shortLink": false,
  "tags": [],
  "posts": [
    {
      "integration": {
        "id": "your-threads-integration-id"
      },
      "value": [
        {
          "content": "1/ Starting a thread on Threads from the API",
          "image": []
        },
        {
          "content": "2/ Each value entry becomes a reply",
          "image": []
        }
      ],
      "settings": {
        "__type": "threads"
      }
    }
  ]
}
```

### Post with Image

```json
{
  "type": "now",
  "date": "2024-12-14T10:00:00.000Z",
  "shortLink": false,
  "tags": [],
  "posts": [
    {
      "integration": {
        "id": "your-threads-integration-id"
      },
      "value": [
        {
          "content": "Sharing today's launch screenshot",
          "image": [
            {
              "id": "image-id",
              "path": "https://uploads.postqueen.ai/launch.png"
            }
          ]
        }
      ],
      "settings": {
        "__type": "threads"
      }
    }
  ]
}
```
