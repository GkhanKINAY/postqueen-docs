...
sidebarTitle: IG Standalone
description: 'API settings for posting to standalone Instagram accounts'
---

Creating the OAuth app and connecting a channel:
[Instagram Standalone setup guide](/installation/providers/instagram#instagram-standalone-option). It shares a
page with Instagram, because both connectors are created from the same Meta app.
...
everything else (post type, collaborators, image arrays) is
identical.
...
ignored when posting, and the `audioSearch` tool is not offered for
