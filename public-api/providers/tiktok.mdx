...
Creating the OAuth app and connecting a channel: [TikTok setup guide](/providers/tiktok).
...
`DIRECT_POST`. With `UPLOAD` the field is still required by the PostQueen API, but TikTok silently
...
silently discarded, as [`content_posting_method`](#content_posting_method) explains.
