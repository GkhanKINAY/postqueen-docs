...
{/* OVERHAUL-TODO(B5): write it from _templates/api-settings.mdx. Sources: this file (fix; WIP: partly better; audit: api). Row in page-status-2026-09-23.md. */}

Creating the OAuth app and connecting a channel: [Skool setup guide](/channels/skool).
...
The Skool group ID where the post will be published. You can find available groups by selecting them in the PostQueen UI when configuring the Skool channel.

### `label`

The label ID to categorize the post. Each Skool group can have multiple labels, so select the target label in the PostQueen UI to obtain its ID.
      ...
      "value": [
        {
          ...
          "image": [
            {
              "id": "metrics-image-id",
              "path": "https://uploads.postqueen.ai/metrics.png"
            }
          ]
        }
      ],
...
<Note>
Skool uses the PostQueen Chrome Extension for authentication. Make sure the extension is installed and configured before using the API. Setup: [Skool provider guide](/channels/skool).
</Note>
