...
{/* OVERHAUL-TODO(B8): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file, moved from automation/n8n (fix; WIP: better, use as base; audit: cli-help). Row in page-status-2026-09-23.md. */}
...
| Field | Description |
| --- | --- |
| **API Key** | Your PostQueen Public API key. In [app.postqueen.ai](https://app.postqueen.ai), open **Connections > API Keys** and hit **Reveal**. Only a workspace admin can reveal the key. |
...
| --- | --- |
| **Video Type** | The type to render, for example `image-text-slides` or `seedance`. |
...
prompt, as `seedance` or `image-text-slides`, vertical or horizontal. Add a voiceover if you want one.
...
<CardGroup cols={2}>
  ...
  <Card title="Create Post" icon="send" href="/public-api/posts/create">
