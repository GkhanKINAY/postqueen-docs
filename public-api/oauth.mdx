...
<Steps>
  <Step title="Register Your OAuth App">
    ...
    <Note>
    Each organization has one OAuth application. Creating or rotating the client secret is an admin action on a plan that includes the public API. The API key itself lives on **Settings → API Keys**, which is a different tab.
    </Note>
