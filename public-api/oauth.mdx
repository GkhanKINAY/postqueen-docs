...
description: 'Let users authorize your app to access PostQueen on their behalf'
---
...
OAuth2 lets you build third-party apps and agents that act on behalf of PostQueen users. Instead of asking users for their API key, your app redirects them to PostQueen where they approve access on a consent screen, and you receive a `pos_` access token. PostQueen supports the OAuth2 Authorization Code flow.
...
sequenceDiagram
    ...
    User->>YourApp: Clicks "Connect with PostQueen"
    YourApp->>PostQueen: Redirect to /oauth/authorize
    PostQueen->>User: Shows consent screen
    User->>PostQueen: Approves access
    PostQueen->>YourApp: Redirects with ?code=...
    YourApp->>PostQueen: POST /oauth/token (exchange code)
    PostQueen->>YourApp: Returns access_token
    YourApp->>PostQueen: API calls with access_token
```
...
<Steps>
  <Step title="Register Your OAuth App">
    Go to **Settings → Developers** in your PostQueen dashboard and create an OAuth application.

    <Note>
    Each organization has one OAuth application. Creating or rotating the client secret is an admin action on a plan that includes the public API. The API key itself is under **Connections > API Keys**, which is a different screen.
    </Note>
  ...
  <Step title="Redirect Users to Authorize">
    When a user wants to connect their PostQueen account to your app, redirect them to:

    ```
    https://app.postqueen.ai/oauth/authorize?client_id={CLIENT_ID}&response_type=code&state={STATE}
    ```

    Self-hosting? Use `https://postqueen.example.com/oauth/authorize` instead.
    ...
    **Example:**
    ...
    ```bash
    https://app.postqueen.ai/oauth/authorize?client_id=pca_VklHTpdEJ6dJ73FHQEJ97qVA0lcMDsrs&response_type=code&state=random123
    ```
  ...
  <Step title="Catch the callback">
    After the user makes a decision, PostQueen redirects them to your **Redirect URL** with query parameters.
    ...
    **If denied:**
    ...
    <Tip>
    Verify that the `state` parameter matches what you originally sent to prevent CSRF attacks.
    </Tip>
  </Step>
  ...
  <Step title="Exchange Code for Token">
    ...
    Self-hosting? The token endpoint is `https://postqueen.example.com/api/oauth/token`.
    ...
    **Response:**
    ...
    <Check>
    An `access_token` in the response means the grant is complete. Store the token, not the code: the code is spent.
    </Check>
...
If your client secret is compromised, go to **Settings → Developers** and click **Rotate Secret**. This invalidates the old secret immediately, and any token exchange requests using the old secret will fail.
...
<Accordion title="Express app: redirect, callback, token exchange, first API call" icon="package">
...
const POSTQUEEN_URL = 'https://app.postqueen.ai';
const BACKEND_URL = 'https://api.postqueen.ai';
const REDIRECT_URL = 'https://yourapp.com/callback';

// Step 2: Redirect user to PostQueen
app.get('/connect', (req, res) => {
  ...
  res.redirect(`${POSTQUEEN_URL}/oauth/authorize?${params}`);
});
...
<CardGroup cols={2}>
  <Card title="Check a client_id first" icon="user-check" href="/public-api/oauth/authorize">
    `GET /oauth/authorize`, the call behind the consent screen.
  </Card>
  <Card title="Exchange the code" icon="key" href="/public-api/oauth/token">
    `POST /oauth/token`, field by field, with its failure modes.
  </Card>
  <Card title="List the user's channels" icon="plug" href="/public-api/integrations/list">
    The first call most apps make with a fresh `pos_` token.
  </Card>
  <Card title="Rate limits and errors" icon="code" href="/public-api/introduction">
