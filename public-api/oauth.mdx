...
OAuth2 lets you build third-party apps and agents that act on behalf of PostQueen users. After a user approves your app on a consent screen, you receive a `pos_` access token that works on the Public API: list their connected channels, create and schedule posts, and pull analytics for their accounts. Tokens stay valid until the user revokes them, which they can do at any time from **Settings > Approved Apps** in their dashboard.

## Overview

PostQueen supports OAuth2 Authorization Code flow, allowing you to build third-party applications that act on behalf of PostQueen users. Instead of asking users for their API key, your app redirects them to PostQueen where they approve access, and you receive a token to make API calls on their behalf.
