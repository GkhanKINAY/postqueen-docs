...
description: 'Fetch a public media URL into the library and return a hosted path.'
...
- PostQueen performs an [SSRF-safe fetch](https://github.com/nodejs/undici): private IP ranges, link-local addresses, and `localhost` are rejected. A self-hosted instance on a trusted private network can switch this off with [`DISABLE_SSRF_PROTECTION`](/configuration/reference#disable_ssrf_protection).
...
Common failure modes: [Uploads troubleshooting](/troubleshooting/common-errors).
