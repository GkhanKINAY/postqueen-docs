...
Threads, the standard TikTok connection, Telegram, Mastodon, Bluesky, Nostr, VK, Kick, Tumblr, Moltbook and
...
| Tool | Parameters | Returns |
|---|---|---|
| `musicSearch` | `genre`, optional, for example POP, ROCK or ELECTRONIC | Tracks from TikTok's trending commercial music list, as `{ id, title, artist, image, duration, previewUrl }`, with `duration` in milliseconds. There is no text search, so filter the list yourself. Pass the id as `music.id` in the [post settings](/public-api/providers/tiktok). |
| `locationSearch` | `q` | Location tags matching the search, as `{ id, name, address }`. An empty `q` returns an empty list. Pass `id` and `name` as the `location` setting. |
...
<CardGroup cols={2}>
  <Card title="Call a tool" icon="zap" href="/public-api/integrations/trigger">
    `POST /integration-trigger/{id}`, the endpoint behind every example here.
  </Card>
  <Card title="Ask one channel directly" icon="sliders-horizontal" href="/public-api/integrations/settings">
    The live `tools` array for a connected channel, parameter schema included.
  </Card>
  <Card title="Use the values in a post" icon="/images/channels/reddit.svg" href="/public-api/providers/reddit">
    Where a subreddit and its flair go once you have fetched them.
  </Card>
  <Card title="Run tools from the CLI" icon="terminal" href="/cli/managing-posts">
