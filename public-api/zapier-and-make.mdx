...
Creating posts has an hourly limit per workspace. A Zap or scenario that loops over many rows can reach it, so put several channels in one request where you can. The limits and the error codes are on [Limits and errors](/public-api/limits-and-errors).
