...
<CardGroup cols={2}>
  ...
  <Card title="RSS AutoPost" icon="rss" href="/automation/rss-autopost">
    Auto-publish new items from RSS feeds to your channels
  </Card>
</CardGroup>

## Authentication

There are two ways to authenticate with the PostQueen API:

### API Key

Get your API key from **Settings**, then **Developers**, then **Public API**. The Developers tab appears for organization admins on a plan that includes the public API. Include the key in the `Authorization` header:
...
If you're building an app for other PostQueen users, use [OAuth2 Authentication](/public-api/oauth) to get tokens that act on behalf of users. OAuth tokens start with `pos_` and are used the same way:
...
| Self-hosted | `{NEXT_PUBLIC_BACKEND_URL}/public/v1`, for example `https://your-domain.com/api/public/v1` |
...
The rate limit is a single global value for the whole instance, and it doesn't tier by subscription plan. Plans tier on channel and post-per-month quotas instead. Self-hosters can adjust the per-hour limit with the `API_LIMIT` env var (see [Configuration Reference](/configuration/reference)). The official Docker Compose sets `API_LIMIT: 30`; with the variable unset the backend defaults to 90 requests per hour.
...
It's the same wizard to schedule posts in the PostQueen app, however instead of scheduling posts, it generates the JSON payload for you to use in your API requests.
  ...
  <Tab title="Local (Make sure your local server is running)">
      <iframe src="http://localhost:4200/modal/dark/all" style={{zoom: 0.5, height: 1000, width: '100%'}} />
        ...
        <p style={{fontSize: '0.85em', textAlign: 'center', marginTop: '6px'}}>
          Source-mode default. If you're running the bundled Docker image (<code>ghcr.io/gkhankinay/postqueen-app</code>), the official compose maps to <code>http://localhost:4007</code>.
        </p>
  </Tab>
</Tabs>
...
PostQueen posts to 30+ networks. When creating posts, each platform has its own settings schema, and the `settings` object must include a `__type` field matching the provider.
...
<Tabs>
  <Tab title="Social">
    ...
    The Farcaster identifier is spelled `wrapcast` (not `warpcast`); the API rejects any other spelling.
  </Tab>
  <Tab title="Video">
    | Platform | `__type` | Key settings |
    |----------|----------|--------------|
