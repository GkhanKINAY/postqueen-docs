---
title: API Overview
description: 'REST API for integrations, posts, uploads, analytics and OAuth'
keywords:
  - postqueen
  - public api
  - openapi
...
{/* OVERHAUL-TODO(B8): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (rewrite; WIP: partly better, and it is the base; audit: api). Row in page-status-2026-09-23.md. */}
...
<CardGroup cols={2}>
  <Card title="NodeJS SDK" icon="package" href="/public-api/sdk">
    Call the API from Node with the official typed client.
  </Card>
  <Card title="n8n Node" icon="/images/brand/icon-n8n.svg" href="/public-api/n8n">
    Drop the community node into any workflow, no code needed.
  </Card>
  <Card title="Zapier" icon="/images/brand/icon-zapier.svg" href="/public-api/zapier-and-make">
    Wire posts into thousands of apps with a webhook and your key.
  </Card>
  <Card title="Make" icon="/images/brand/icon-make.svg" href="/public-api/zapier-and-make">
  ...
  <Card title="RSS AutoPost" icon="rss" href="/using/rss-auto-post">
...
Get your API key from **Connections > API Keys** in the app. The key belongs to the whole workspace, so only a workspace admin can reveal or rotate it; members see it masked. Include the key in the `Authorization` header, with no `Bearer` prefix:
...
| Rotating it | An admin rotates it under **Connections > API Keys**, which takes the old one out immediately | The user who authorized you revokes the app under **Connections > Approved Apps** |
...
<Info>
**An hourly limit per workspace** applies only to the create post endpoint (`POST /posts`). Uploads have a separate hourly allowance of their own.

That is not a limit on posts. Each call counts once no matter how much it carries, so put several posts in one request when you need volume.
</Info>

The rate limit is one global value for the whole instance, and it is the same on every subscription plan. Plans differ on channel counts and posts per month instead. Self-hosting? You set the per-hour limit with the `API_LIMIT` env var, and the upload allowance with `UPLOAD_LIMIT` (default 300). The official Docker Compose sets `API_LIMIT: 30`; with the variable unset the backend defaults to 90 requests per hour. Full env var list: [Configuration Reference](/configuration/reference).
...
| `413 Payload Too Large` | Your request body exceeded the default JSON limit of roughly 100 kB. The 50 MB limit you may have read about is on the internal routes the web UI uses, not on this one, so inlining a single base64 image is enough to trip it. Upload first and send the returned `path`. See [Uploads troubleshooting](/troubleshooting/common-errors). |
| `429 Too Many Requests` | You exceeded `API_LIMIT` per hour on the create-post endpoint, or `UPLOAD_LIMIT` on uploads, which are counted separately. |
...
<CardGroup cols={2}>
  <Card title="Schedule your first post" icon="send" href="/public-api/posts/create">
  ...
  <Card title="Use the typed client" icon="box" href="/public-api/sdk">
