...
Get your API key from **Settings → API Keys**. The tab is there for every member of the workspace; revealing or rotating the key is an admin action. Include the key in the `Authorization` header, with no `Bearer` prefix:
...
| Rotating it | Regenerate in **Settings → API Keys**, which takes the old one out immediately | Rotate the client secret in **Settings → Developers** |
