...
<Info>
**30 requests per hour** limit applies only to the create post endpoint and, in a separate allowance of the same size, to uploads.
...
| `429 Too Many Requests` | You exceeded `API_LIMIT` per hour on the create-post endpoint, or on uploads, which are counted separately. |
