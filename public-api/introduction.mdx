...
| `403 Forbidden` | The API key is valid but does not own the resource (e.g. you tried to delete a post in another organization). |
...
She posts to 30+ networks, and each one has its own settings schema. Every post you send carries a `settings` object, and that object needs a `__type` field naming the provider.
