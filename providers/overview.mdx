...
description: 'Configure social media providers for PostQueen'
---

You can see all the providers that PostQueen supports documented in the sidebar, under "**Providers**".

Please note that no providers are configured by default. You will need to configure them all in your `.env` file, or as environment variables. You will need to restart PostQueen whenever you change environment variables. If you are using docker compose, you must run `docker compose down` and then `docker compose up` to rebuild the containers with the updated variables.
