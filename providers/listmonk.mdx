...
**Select List** and **Select Template** render nothing at all when the call behind them comes back empty. That is an empty account or an API user without access to those endpoints, not a broken editor. Create a list in Listmonk, or widen the user's access, then reopen the post.
