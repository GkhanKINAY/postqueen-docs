...
`is_flair_required` is not optional. Leave it out and the call is refused before the post is created. Say `false` when the subreddit does not need a flair, and drop the `flair` object with it.
...
`restrictions` answers with an object, not a list: `.output.flairs[]` holds the flairs, and `.output.is_flair_required` tells you whether the subreddit insists on one. A flair you send needs both its `id` and its `name`.
...
TikTok is the strictest settings block in the CLI. Seven of these fields are required and there are no defaults, so a shorter object is refused rather than filled in: `content_posting_method`, `privacy_level`, `duet`, `stitch`, `comment`, `autoAddMusic`, `brand_content_toggle` and `brand_organic_toggle`. Only `video_made_with_ai` is optional.
