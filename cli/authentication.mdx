...
If you run a self-hosted PostQueen instance, also point the CLI at your own backend, the same
value as `NEXT_PUBLIC_BACKEND_URL`. The CLI appends `/public/v1/...` to whatever you set, and
most single-domain setups serve the backend under `/api`:
...
Your channels come back as JSON. That is the check that matters, because it is the first command
...
configured with", not "does this still work".
</Note>

Because `auth:status` never contacts the server, the only real proof is a command that does:
...
If you want the device flow anyway, for example to hand out short-lived CLI access without sharing API keys, you can self-host the auth server, and the setup is written out in full from here on.
