...
postqueen integrations:list | jq '.[] | {id, identifier}'
```

```bash
# Find a specific platform
postqueen integrations:list | jq '.[] | select(.identifier=="reddit")'
```
...
GROUP_ID=$(postqueen integrations:groups | jq -r '.[] | select(.name=="My Company") | .id')
...
INTEGRATION_ID=$(postqueen integrations:list | jq -r '.[] | select(.identifier=="reddit") | .id')
