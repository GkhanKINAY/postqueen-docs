...
{/* OVERHAUL-TODO(B6): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: better, use as base; audit: cli-help). Row in page-status-2026-09-23.md. */}
...
|----------|---------|--------------|
| `POSTQUEEN_API_KEY` | - | Your API key. Get it from [app.postqueen.ai](https://app.postqueen.ai): **Connections > API Keys**, then **Reveal**. Only a workspace admin can reveal it. |
...
| `postqueen auth:login` | `--auth-server <url>` (no default) | Without an auth server it makes no network call: it explains where the API key is and how to set `POSTQUEEN_API_KEY`, and exits `1` unless that key is set. With `--auth-server` (or `POSTQUEEN_AUTH_SERVER`) it runs the OAuth2 device flow against a server you run yourself and saves the token to `~/.postqueen/credentials.json` with `0600` permissions. |
...
<Warning>
On the hosted service the CLI signs in with an API key only. PostQueen runs no auth server; the device flow is only for one you run yourself ([Authentication](/cli/introduction)). Every command works identically either way.
</Warning>
...
| `postqueen posts:settings <id>` | `--settings <json>` (required) | Merges provider-specific settings onto a draft or unpublished scheduled post. Only the keys you pass change. Do not include `__type`. The same operation as [`PUT /posts/{id}/settings`](/public-api/posts/settings). |
...
# Change provider settings on a draft or unpublished scheduled post
postqueen posts:settings post-123 --settings '{"content_posting_method":"DIRECT_POST"}'
```
...
<Note>
`posts:create` counts against an hourly limit per workspace on `POST /posts`. Running your own instance? Raise `API_LIMIT`.
</Note>
...
**Anything that fails exits `1`.** Errors go to stderr behind a `❌` and results stay on stdout, so a failed command falls out of your pipeline instead of feeding it half a result. `set -e` catches it. Every message and its cause: [Troubleshooting](/agents/troubleshooting).
...
<CardGroup cols={2}>
  ...
  <Card title="Platform Examples" icon="layout-grid" href="/cli/platform-examples">
