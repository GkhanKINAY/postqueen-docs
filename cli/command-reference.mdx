...
| `POSTQUEEN_AUTH_SERVER` | none | A device-flow auth server you run yourself. PostQueen runs none, so there is no default; see [Authentication](/cli/authentication). |
...
<Warning>
On the hosted service the CLI signs in with an API key only. PostQueen runs no auth server; the device flow is only for one you run yourself ([Authentication](/cli/authentication)). Every command works identically either way.
</Warning>
