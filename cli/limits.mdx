...
The CLI reaches fourteen of the API's twenty-three routes. Here is everything it does not, and
...
| Confirm your key still reaches her | `auth:status` prints local credentials, then calls `GET /public/v1/integrations` and reports the channel count | [`GET /is-connected`](/public-api/integrations/is-connected), or `postqueen integrations:list` |
...
### `auth:status` verifies the key

It prints which credentials the CLI found, which method it will use and which API URL it will
call, then `GET /public/v1/integrations` to prove the key still works. A revoked key fails that
call. The same integrations list is what `postqueen integrations:list` prints if you want the
channels themselves.

### Analytics reach further here than anywhere else

`analytics:platform` reads channel numbers such as follower growth, which
[MCP](/mcp/limits-and-troubleshooting) does not carry (MCP reports post analytics only), and n8n has no
analytics operation. If reading channel numbers is part of the job, the CLI or the API are the two
surfaces that can do it.
...
<CardGroup cols={2}>
  ...
  <Card title="Command reference" icon="terminal" href="/cli/command-reference">
    The seventeen commands that do exist, with their flags
  </Card>
