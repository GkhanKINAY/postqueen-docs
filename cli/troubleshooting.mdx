...
| `API Error (401): {"msg":"Invalid API key"}` | The key is wrong, revoked, or has whitespace. | Re-copy it from **Connections > API Keys**. Only a workspace admin can reveal the key, so a member asks an admin. |
...
| `No auth server is answering at <url> (<detail>).` | `auth:login` was pointed at an auth server with `--auth-server` or `POSTQUEEN_AUTH_SERVER`, and nothing answered there. | Check that URL. PostQueen runs no auth server, so on the hosted service use an API key: `export POSTQUEEN_API_KEY=your_api_key` ([Authentication](/cli/authentication)). |
...
| `API Error (429): ... "ThrottlerException: Too Many Requests"` | You crossed the hourly limit per workspace on `POST /posts`. | Wait for the window to roll, or batch multiple channels into one `posts:create` with a comma-separated `-i`. Running your own instance? Raise `API_LIMIT`. |
