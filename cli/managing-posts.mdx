...
{/* OVERHAUL-TODO(B6): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: better, use as base; audit: cli-help), main:cli/integrations.mdx (fix; WIP: better, use as base; audit: cli-help). Row in page-status-2026-09-23.md. */}
...
postqueen posts:create \
  ...
  -s "2026-08-01T09:00:00Z" \
  --settings '{"subreddit":[{"value":{"subreddit":"/r/programming","title":"My Post","type":"self","url":"","is_flair_required":false}}]}' \
  -i "reddit-integration-id"
```

<Tip>
Use `postqueen integrations:settings <id>` to discover what settings are available for each platform. Full walkthrough: [Integrations](/cli/managing-posts).
</Tip>
