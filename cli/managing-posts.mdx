...
Each comment can have its own media with a corresponding `-m` flag:
...
The file lists the target `integrations` and a `posts` entry per provider. Each provider entry has its own `post` array (the first item is the main post, additional items become comments/threads) and an optional `settings` object. Provider settings use a discriminator pattern: the `__type` field tells the backend which platform's settings schema to apply.
...
<Note>
When you pass `--settings` on the command line instead, you can omit `__type` — the backend adds it automatically based on the integration ID. In a `--json` file, include `__type` explicitly for each provider's settings.
</Note>
...
postqueen posts:list | jq '.posts[] | select(.releaseId == "missing") | {id, content}'
