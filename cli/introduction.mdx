...
If an AI assistant will be driving the CLI for you, install the PostQueen skill as well. The skill is a playbook (`SKILL.md`): it teaches the agent her flags and patterns so it gets commands right the first time. It does **not** install the CLI. You still need `npm install -g postqueen` from the step above.
...
| `posts:settings <id>` | Merge provider settings onto a draft or unpublished scheduled post with `--settings` |
...
<CardGroup cols={2}>
  ...
  <Card title="What it cannot do" icon="gauge" href="/cli/limits">
    Fourteen of the API's twenty-two endpoints, and where the other eight live.
  </Card>
</CardGroup>
