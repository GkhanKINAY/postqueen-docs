...
{/* OVERHAUL-TODO(B5): write it from _templates/channel.mdx. Sources: this file, moved from providers/slack (fix; WIP: better, use as base; audit: providers). Row in page-status-2026-09-23.md. */}
...
Nine networks report numbers back to the Analytics page. Slack is not one of them, so a Slack channel shows no chart. The message body goes up as a Slack markdown block, and PostQueen reads back the permalink so the post links straight to the message in your workspace.
