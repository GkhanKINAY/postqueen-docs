...
{/* OVERHAUL-TODO(B2): write it from _templates/channel.mdx. Sources: this file, moved from providers/x-twitter (fix; WIP: better, use as base; audit: providers). Row in page-status-2026-09-23.md. */}

<img src="/images/channels/x.svg" alt="X" height="44" noZoom style={{ height: "44px", marginBottom: "1.25rem" }} />

<Warning>
**In review.** Connects once your trial ends. X's review of PostQueen's app is in progress. [Status of every network](/channels/overview#status-on-postqueen)
</Warning>
...
<CardGroup cols={2}>
  <Card title="Character limit" icon="type">
    280 characters, or 4,000 on a channel marked Verified. An article takes up to 100,000
  </Card>
  <Card title="Media" icon="images">
    Optional. Up to 4 pictures, or one video. Pictures only in an article
  </Card>
  <Card title="Post types" icon="layers">
    Post, reply, thread, community post, long-form article
  </Card>
...
| | |
|---|---|
| **Post settings** | `who_can_reply_post`, `community`, `made_with_ai`, `paid_partnership`, plus `post_type`, `article_title`, `article_status` and `article_cover` for an article. Full schema in the [X settings reference](/public-api/providers/x) |
...
- **Post type**: Switch from a regular post to **Article (long-form)** and she publishes an X article instead. You give it a title, choose whether it goes out published or lands in your X drafts, and can add a cover image. Articles keep their headings, lists and links, take pictures but no video, and a draft article cannot carry thread replies.
