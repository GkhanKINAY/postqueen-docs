...
PostQueen refuses a post that breaks these rules and shows the message in the composer. The post and every reply in the thread are checked on their own.
...
<Steps>
  ...
  <Step title="Sign in to Threads">
    Sign in with the Threads profile you want to post to, and approve the permissions.
  </Step>
...
The connection renews itself, so you do not need to reconnect it on a schedule.
...
The composer shows an **Add a thread finisher** switch for Threads. PostQueen does not publish it, so write a closing post as the last part of the thread instead.
...
In the channel's **Automations** on the **Channels** page, **Auto plug post** adds a reply you write in advance once the post passes a number of likes you set. It checks the post every 6 hours, up to 3 times.
