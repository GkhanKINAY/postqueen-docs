...
{/* OVERHAUL-TODO(B5): write it from _templates/channel.mdx. Sources: this file, moved from providers/discord (fix; WIP: better, use as base; audit: providers). Row in page-status-2026-09-23.md. */}
...
Nine networks report numbers back to the Analytics page. Discord is not one of them, so a Discord channel shows no chart. What it does carry is the markdown editor: headings, quotes and code fences survive the trip exactly as you wrote them. Discord rate limits are generous enough that PostQueen runs five posting jobs side by side.
