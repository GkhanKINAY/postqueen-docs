...
its own. Your connected channels come back as JSON with their ids. Doing this first separates a
credential problem from a workflow problem, which is much harder to tell apart later.
...
**Generate Video** starts with a **Video Type**, for example `image-text-slides` or `veo3`. Then pick an **Output Format**: `vertical` or `horizontal`. **Custom Parameters** are optional key/value pairs such as `prompt`, `voice` or `images`. Need the list of voices? Run **Video Function** with the function name `loadVoices` and the identifier of the video type you plan to use, such as `image-text-slides`. Both fields are required on that operation.
...
She runs on your terms. Leave her alone and she is a true autopilot: whatever lands on the calendar goes out on time without another word from you. Want a closer look first? Everything is visible on the calendar before it publishes, and setting **Create Post** to draft saves the post without scheduling it, so it stays there until you publish it.
