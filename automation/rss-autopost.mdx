...
An AutoPost item is created as a draft, so she holds it until you open it and schedule or publish it. Each hourly check takes one item, the newest one, so a feed that publishes in bursts leaves the older items behind.
