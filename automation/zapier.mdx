...
icon: /images/brand/icon-zapier.svg
---
...
<Frame>
  <img src="/images/brand/auto-zapier.svg" alt="A Zapier trigger feeding a Webhooks by Zapier step that schedules a post on the PostQueen calendar" />
</Frame>

## Before you start

Grab your API key from [app.postqueen.ai](https://app.postqueen.ai): open **Connections > API Keys** and hit **Reveal**. Only a workspace admin can reveal the key. Treat it like a password and paste it only into the Zap step that needs it.
...
<Warning>
The create-post endpoint has an hourly limit per workspace. A Zap that fires on a busy trigger can hit that ceiling, so add a filter or a delay when your source is chatty. On self-hosted instances the limit is the `API_LIMIT` environment variable.
</Warning>
