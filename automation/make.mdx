...
icon: /images/brand/icon-make.svg
---
...
<Frame>
  <img src="/images/brand/auto-make.svg" alt="A Make scenario whose HTTP module sends a post to PostQueen and schedules it on the calendar" />
</Frame>

## Before you start

Your Public API key. In [app.postqueen.ai](https://app.postqueen.ai), open **Connections > API Keys** and click **Reveal**. Only a workspace admin can reveal the key.
...
One thing to watch: the create-post endpoint has an hourly limit per workspace. A scenario that loops over many rows can hit the ceiling, so batch several posts into one request where you can, or add a **Sleep** module between iterations.
