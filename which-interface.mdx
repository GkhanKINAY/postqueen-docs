...
| **[MCP](/mcp/introduction)** | Nothing for most clients, just one URL. Claude Desktop takes a **custom connector**, or `mcp-remote` in `claude_desktop_config.json` for a LAN/VPN instance. A plain `"url"` in that file does not work | API key as a Bearer token on `/mcp`, or the key in the URL at `/mcp/:apiKey`. OAuth tokens starting with `pos_` work on the Bearer form only | **14 tools** on `/mcp` (channel discovery, scheduling, media generation, `ask_postqueen`). Directory OAuth paths expose fewer: [tool counts by path](/mcp/introduction#how-many-tools-you-actually-get). Upload from a URL too. Lists scheduled posts and edits their settings. No analytics, and no deleting or rescheduling posts | Claude, ChatGPT, Cursor, Grok or any MCP client where you would rather ask than type flags |
...
Point your client at a single URL and 14 tools arrive with their schemas. Among the four routes, AI
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/cli/introduction">
    Install it globally, export your key, and 17 commands are one line away.
  </Card>
  <Card title="MCP" icon="plug" href="/mcp/setup">
    One URL for your client, and 14 tools arrive with their schemas.
  </Card>
