...
{/* OVERHAUL-TODO(B9): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: same as main except dropping `version: …; audit: selfhost). Row in page-status-2026-09-23.md. */}
...
[Caddy](/installation/domain-and-https) is less work for the same result.
...
<Steps>
  ...
  <Step title="Tell PostQueen her address">
    ...
    origins and every social network's return address are built from it. Put them in `.env`:
  ...
  <Step title="Check it">
    ...
    **This block does not get you a real certificate.** `tls=true` turns TLS on, but with no
    ...
    [Caddy](/installation/domain-and-https) does this part for you, which is why it is the shorter path.
    </Warning>
...
<CardGroup cols={2}>
  <Card title="Caddy" icon="shield" href="/installation/domain-and-https">
  ...
  <Card title="WebSockets and dev tunnels" icon="bolt" href="/reverse-proxies/split-deployments">
