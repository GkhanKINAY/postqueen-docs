...
[Caddy](/installation/domain-and-https) and handles certificates through a separate tool, so it is the
...
[Docker on its own](/installation/docker-compose) she is on `localhost:5000` instead, so change the
...
```
  ...
  <Step title="Tell PostQueen her address">
    ...
    sign-in origins and every social network's return address from these values. Put them in
    `.env`:
