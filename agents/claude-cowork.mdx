...
<Steps>
  <Step title="Add the PostQueen connector in Claude">
    Skip this step if Claude already has it. In Claude, open **Customize > Connectors**, then **+ > Add custom connector**. Name it `PostQueen`, paste your key address as the URL, with no slash at the end, and click **Add**:

    ```text wrap
    https://api.postqueen.ai/mcp/YOUR_API_KEY
    ```

    [The Claude page](/agents/claude) has the full steps, including the Team and Enterprise ones.
  </Step>
...
| The key is refused | Copy the address again from **Connections**. Only a workspace Admin or Super Admin can reveal the key. |
