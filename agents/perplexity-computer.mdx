...
| Way | Use it with Perplexity Computer? |
|---|---|
| The key in the address, `{{mcpKeyUrl}}` | **Yes.** The route above |
| The Bearer header on `{{mcpBearerUrl}}` | Not documented. Perplexity does not say which header its API Key option sends |
| Sign in with PostQueen | Not supported with Perplexity yet. Use the key address |
...
| The connection is refused | Use the key address with **Authentication** set to **None**, and copy the key again from **Connections > API Keys**. |
...
No. This page is about Computer, Perplexity's agent. Two other Perplexity products share part of the name:

- **Perplexity search** is Perplexity's answer engine. This page covers Computer, the agent that carries out tasks.
