...
Gemini CLI reads MCP servers from its `settings.json`, and she is already hosted at a URL, so nothing new runs on your machine. On connect it discovers her [14 tools](/mcp/tools) and reads their schemas, which is why you never type a tool call by hand.

Those 14 tools cover discovery, media and posting, and they can read back what is already scheduled and change a scheduled post's settings. They do not delete a post or move it to a new time, and there is no analytics tool among them. Both of those live on the calendar at [app.postqueen.ai](https://app.postqueen.ai) and in the [`postqueen` CLI](/cli/introduction), which Gemini CLI can run as a shell command like any other.
...
**Settings → API Keys**. Then run `/mcp` inside Gemini CLI: `postqueen` should
show as connected with 14 tools.
...
4. `schedulePostTool` writes the calendar entries, one item in `socialPost` per post:
...
<AccordionGroup>
  ...
  <Accordion title="What can she do from a Gemini CLI session?">
    Everything her 14 MCP tools cover. She lists your channels, reads what each platform accepts and schedules posts across your 30+ networks. Along the way she can fetch dynamic values like subreddit flairs, generate an image or build a video to attach.
  </Accordion>
  <Accordion title="Can it check how a post performed?">
    Not over MCP. There is no analytics tool in the set, and no tool for deleting posts either. For those, use the calendar at [app.postqueen.ai](https://app.postqueen.ai) or the `postqueen` CLI, which carries `posts:delete` and `analytics:platform`.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="MCP tools" icon="wrench" href="/mcp/tools">
    All 14 tools Gemini discovers on connect, with every parameter.
  </Card>
