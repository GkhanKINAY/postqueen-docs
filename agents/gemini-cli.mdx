---
title: 'Gemini CLI'
description: 'One command connects Gemini CLI to her hosted MCP server, and the terminal you already have open can put finished posts on your calendar'
---
...
Gemini CLI reads MCP servers from its `settings.json`, and she is a hosted server, so nothing new runs on your machine. On connect it discovers her [10 tools](/mcp/tools) and reads their schemas, which is why you never type a tool call by hand.

Those 10 tools cover discovery, media and posting. They do not list, delete or reschedule posts that already exist, and there is no analytics tool among them. Both of those live on the calendar at [app.postqueen.ai](https://app.postqueen.ai) and in the [`postqueen` CLI](/cli/introduction), which Gemini CLI can run as a shell command like any other.
...
<Steps>
  ...
  <Step title="Verify the connection">
    ...
    It lists configured servers with their connection status and discovered tools, and `postqueen` should show as connected with 10 tools. Then ask:
...
<CardGroup cols={2}>
  ...
  <Card title="MCP tools" icon="wrench" href="/mcp/tools">
    All 10 tools Gemini discovers on connect, with every parameter.
  </Card>
