...
Gemini CLI reads MCP servers from its `settings.json`, and she is a hosted server, so nothing new runs on your machine. On connect it discovers her [11 tools](/mcp/tools) and reads their schemas, which is why you never type a tool call by hand.
