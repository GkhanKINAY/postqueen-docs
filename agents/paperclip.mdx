...
| The PostQueen CLI and skill | `npm install -g postqueen` | Not tested yet |
...
Approve the save when Paperclip asks. The drafts appear on your PostQueen calendar, ready for you to check and schedule.
...
| The key is refused | Paste the key on its own, without the word `Bearer`, copied from **Connections > API Keys**. |
