...
These are caught before anything is sent, and the command exits `1`. A message with `❌` is one line on stderr. The others come at the end of the command's whole help, also on stderr.
...
| `All posts must have an integration id` | `-i` has an empty entry, usually from a stray or trailing comma such as `-i "id1,"` |
...
<AccordionGroup>
  ...
  <Accordion title="Nothing happened, and the exit code is 0" icon="circle-question-mark">
    The CLI does not refuse a command or a flag it does not know. `postqueen post:create` prints nothing and exits `0`, and `posts:list --startdate ...` ignores the flag and lists the default window. Check the spelling against `postqueen <command> --help`: flag names are case-sensitive, so `--startDate` and `--start-date` work and `--startdate` does not.
  </Accordion>
  <Accordion title="Text that starts with a dash" icon="minus">
    A value that starts with `-` is read as more flags, so the text never arrives: `-c "-hello"` prints the help and exits `0`. Attach it with `=`: `--content="-5% off today"` or `-c="- a smaller size"`. See [text that starts with a dash](/cli/command-reference#text-that-starts-with-a-dash).
  </Accordion>
