...
<AccordionGroup>
  <Accordion title="The connection fails with Invalid API Key" icon="key-round">
    The key address answers `400` with `Invalid API Key` when it does not know the key. Copy the key again from **Connections > API Keys** and paste the whole address, `https://api.postqueen.ai/mcp/YOUR_API_KEY`, with no space or quote around the key. Only a workspace admin can reveal the key; other members see it masked.
  </Accordion>
  <Accordion title="The connection fails with 401" icon="shield-x">
    On `https://api.postqueen.ai/mcp`, `Missing Authorization header` means the agent sent no header, and `Invalid API Key or OAuth token` means the key in it is wrong. The header must read `Authorization: Bearer YOUR_API_KEY`.
  </Accordion>
  ...
  <Accordion title="The sign-in fails" icon="lock-keyhole">
    ...
    - Gemini CLI, Grok, Grok Bot, Perplexity Computer and NanoClaw take the key address only.
...
| `No posts are left on this plan this month. ...` | The workspace has no active trial or plan | Start or renew the plan in **Billing**. See [plans](/cloud/plans) |
...
```text
❌ Failed to create post: Request failed: API Error (400): {"statusCode":400,"provider":"x","name":"PostQueen Bakery","message":"post is too long, please fix it"}
```

`API Error (401)` is the key, `404` a wrong ID, `400` a post or setting the API refused, and `429` the hourly limit. `Request failed` with no `API Error` means the API was not reached: check your connection.
...
<AccordionGroup>
  ...
  <Accordion title="auth:status says the key is fine, the script still stops" icon="circle-alert">
    `auth:status` exits `0` even when it reports a problem, so a script cannot rely on its exit code. Check with `postqueen integrations:list > /dev/null || exit 1` instead.
  </Accordion>
  <Accordion title="A file path or a link in -m" icon="image">
    `-m` takes only a `path` returned by `postqueen upload`. Upload the file first. For a video, wait until it is ready: see [upload media](/cli/media-upload#wait-for-a-video).
  </Accordion>
</AccordionGroup>

## What each route cannot do yet

| You want to | MCP | CLI | Where instead |
|---|---|---|---|
| Delete a post | No | `posts:delete` | The app, the API |
| Change a post's text or time | No | No | The app |
| Move a post between draft and scheduled | No | `posts:status` | The app, the API |
| Read channel numbers, such as followers | No | `analytics:platform` | The app, the API |
| Totals and top posts across the workspace | `analyticsSummaryTool`, `analyticsPostsTool` | No | The app, the API |
| Upload from a link | `uploadFromUrlTool` | No | The API |
| Generate an image | `generateImageTool` | No | The app |
| Connect a channel | No | No | The app |
