...
<Steps>
  ...
  <Step title="Route A: connect over MCP">
    ...
    <Warning>
    A project-level `.cursor/mcp.json` carries your key into the repo. Add it to `.gitignore`, or keep the server in `~/.cursor/mcp.json` and leave the project clean. If the key leaks, regenerate it in **Settings > Developers > Public API** and update every client that uses it.
    </Warning>
  </Step>
...
She runs on your terms. A post you schedule goes out at its time without another word from you, everything is visible on the calendar before it publishes, and drafts wait there for your approval.
