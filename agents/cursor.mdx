...
{/* OVERHAUL-TODO(B7): write it from _templates/agent.mdx. Sources: main:mcp/clients/cursor.mdx (merge; WIP: better, use as base; audit: agents), this file (fix; WIP: better, use as base; audit: agents). Row in page-status-2026-09-23.md. */}
...
Two routes, and you can take either. MCP is the native one here: Cursor's agent gets PostQueen's 21 hosted tools and asks for your approval before it calls them. The postqueen skill plus the `postqueen` CLI is the other, and it suits the agent terminal when you would rather see the exact command that ran.

<Steps>
  <Step title="Grab your API key">
    Your key lives in the app at [app.postqueen.ai](https://app.postqueen.ai): open **Connections > API Keys** and click **Reveal**. Only a workspace admin can reveal the key. Both routes below use this one key. Treat it like a password, since it grants full access to your account.
  </Step>
  <Step title="Route A: connect over MCP">
    One entry in `~/.cursor/mcp.json` and her tools appear in every project. The file, the
    Settings UI route (**Customize → MCP** or **Tools & MCP**), the header variant and self-hosted URLs are all on
    [Cursor over MCP](/agents/cursor).
  ...
  <Step title="Route B: install the CLI and her skill">
    ...
    The skill is her `SKILL.md` playbook: it teaches the agent flags and patterns. It does not install the CLI — `npm install -g postqueen` is a separate step.

    The repo also ships a Cursor plugin at `.cursor-plugin/plugin.json`. To install it locally:
    ...
    Then restart Cursor or run **Developer: Reload Window**. The plugin loads the same skill; it does not register an MCP server.

    Then export the key in the shell Cursor's terminal inherits:
...
<AccordionGroup>
  ...
  <Accordion title="MCP or the CLI in Cursor?">
    MCP is the smoother fit for agent mode: 21 tools, no local install, and an approval prompt before each call. The CLI is better when you want the exact command visible in the terminal, or when you plan to reuse the same script in CI later. Configuring both is fine.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Cursor over MCP" icon="plug" href="/agents/cursor">
