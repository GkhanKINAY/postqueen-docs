...
<Steps>
  ...
  <Step title="Start it">
    VS Code asks you to trust the server before it starts it. Confirm, then open Copilot Chat in agent mode.
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Input variable">
    VS Code recommends input variables for secrets. It asks for the key the first time the server starts and stores it for you, so the file holds no secret:
