...
{/* OVERHAUL-TODO(B7): write it from _templates/agent.mdx. Sources: this file, moved from mcp/clients/vscode (merge; WIP: better, use as base; audit: agents). Row in page-status-2026-09-23.md. */}
...
This page is the VS Code editor. GitHub Copilot CLI (`copilot mcp add`, `~/.copilot/mcp-config.json`) is a different product. Use [Any MCP client](/agents/other-mcp-clients) for that CLI.
...
Most self-hosted setups serve the backend under `/api`. Your reverse proxy has to forward `/mcp` and stream the response: [MCP on a self-hosted install](/configuration/mcp).
