---
title: ChatGPT
description: 'Connect ChatGPT over MCP in Developer mode, or give a Custom GPT an action that schedules posts through her public API'
icon: /images/brand/icon-chatgpt.svg
---

{/* OVERHAUL-TODO(B7): write it from _templates/agent.mdx. Sources: main:mcp/clients/chatgpt.mdx (merge; WIP: better, use as base; audit: agents), this file (rewrite; WIP: better, use as base; audit: agents). Row in page-status-2026-09-23.md. */}

## Working with her in ChatGPT

ChatGPT is where a lot of content gets written. With PostQueen connected it is also where content gets shipped: ask for two posts about the new menu and she writes the copy, generates the images and puts both on your calendar while you keep chatting. This is ChatGPT the chat product, not Codex — same split as Claude vs Claude Code.
...
- **MCP in Developer mode.** PostQueen is not in the ChatGPT app store. One URL under **Settings → Apps → Create**, and PostQueen's [21 MCP tools](/mcp/tools) show up in your chats. This is the route most people want.
...
## Route 1: MCP in Developer mode

ChatGPT reaches her over MCP. She is not listed in the ChatGPT app store; do not search Apps. Turn on **Developer mode** under **Settings → Apps → Advanced settings**, then create an app under **Settings → Apps → Create** with one URL, and she becomes a tool the model can call:

```
https://api.postqueen.ai/mcp/YOUR_API_KEY
```

Your key lives in the app at [app.postqueen.ai](https://app.postqueen.ai): open **Connections > API Keys** and click **Reveal**. Only a workspace admin can reveal the key.

<Warning>
Your API key travels inside that MCP URL, which makes the URL itself a secret. Anyone holding it can post from your accounts. Keep it out of screenshots, and if the key leaks, have an admin rotate it under **Connections > API Keys**, then update every client that uses it.
</Warning>

Custom apps sit behind ChatGPT's developer mode, which OpenAI offers on the paid web plans. Plus and Pro can usually connect; OpenAI currently documents full write MCP (scheduling) for Business and Enterprise/Edu. The full walkthrough covers the developer mode toggle, the create-app form and enabling her per chat:

<Card title="ChatGPT MCP setup" icon="plug" href="/agents/chatgpt">
...
<Note>
The create-post endpoint has an hourly limit per workspace. One request can carry several posts, so a GPT that batches a week into a single call gets a lot done inside that budget. Self-hosting? Point the action at `https://postqueen.example.com/api/public/v1` and tune the limit with the `API_LIMIT` env var.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="What can she actually do from a ChatGPT chat?">
    Everything PostQueen's 21 MCP tools cover, if your ChatGPT plan is allowed to call write tools. Listing channels is a read. Scheduling a post is a write (`integrationSchedulePostTool`). OpenAI currently documents full write MCP for Business and Enterprise/Edu; Plus and Pro may connect but only get read/fetch.
  </Accordion>
  ...
  <Accordion title="Does MCP access cost extra?">
  ...
  <Accordion title="MCP app or Custom GPT action?">
    The MCP app if you want her in your everyday chats with no build step. The action if you are shipping a GPT to other people and want to control exactly which calls it can make.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="MCP tools" icon="wrench" href="/mcp/tools">
    The 21 tools ChatGPT calls once the MCP app is on
  </Card>
