...
- **The connector.** One URL in ChatGPT settings, and her [14 MCP tools](/mcp/tools) show up in your chats. This is the route most people want.
...
ChatGPT reaches her over MCP. Turn on **Developer mode** under **Settings → Apps → Advanced settings**, then create an app under **Settings → Apps → Create** with one URL, and she becomes a tool the model can call:
...
Behind that one sentence sit two tools: `generateImageTool` produces each visual and hands back a hosted URL, then `schedulePostTool` writes the calendar entries. Bringing your own image from somewhere else? `uploadFromUrlTool` pulls it into her media library first, because attachments have to live on a PostQueen URL.
...
<AccordionGroup>
  ...
  <Accordion title="What can she actually do from a ChatGPT chat?">
    Everything her 14 MCP tools cover, if your ChatGPT plan is allowed to call write tools. Listing channels is a read. Scheduling a post is a write (`schedulePostTool`). OpenAI currently documents full write MCP for Business and Enterprise/Edu; Plus and Pro may connect but only get read/fetch.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="MCP tools" icon="wrench" href="/mcp/tools">
    The 14 tools ChatGPT calls once the connector is on
  </Card>
