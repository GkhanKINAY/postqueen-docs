...
- **MCP in Developer mode.** She is not in the ChatGPT app store. One URL under **Settings → Apps → Create**, and her [17 MCP tools](/mcp/tools) show up in your chats. This is the route most people want.
...
Your key lives in the app at [app.postqueen.ai](https://app.postqueen.ai): open **Settings → API Keys** and click **Reveal**.

<Warning>
Your API key travels inside that MCP URL, which makes the URL itself a secret. Anyone holding it can post from your accounts. Keep it out of screenshots, and if the key leaks, regenerate it in **Settings → API Keys** and update every client that uses it.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="What can she actually do from a ChatGPT chat?">
    Everything her 17 MCP tools cover, if your ChatGPT plan is allowed to call write tools. Listing channels is a read. Scheduling a post is a write (`integrationSchedulePostTool`). OpenAI currently documents full write MCP for Business and Enterprise/Edu; Plus and Pro may connect but only get read/fetch.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="MCP tools" icon="wrench" href="/mcp/tools">
    The 17 tools ChatGPT calls once the MCP app is on
  </Card>
