...
<Note>
On the hosted service the CLI signs in with an API key only. `postqueen auth:login` makes no network call unless you point it at a device-flow auth server you run yourself: see [Authentication](/cli/authentication).
</Note>
