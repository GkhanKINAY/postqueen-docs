...
<div className="pq-visual pq-tint-lilac">
  <img noZoom src="/images/site/agents/claude.webp" alt="A sample Claude chat with PostQueen connected: a request for posts, the PostQueen tools it used, and the reply" />
</div>
...
These steps use the key address. To sign in instead of using a key, see [Other ways to connect](#other-ways-to-connect).
...
- **Anthropic advises against secrets in a URL.** The key address works, but it carries your key, so keep it private. The sign-in under [Other ways to connect](#other-ways-to-connect) avoids a key in the URL. If the address leaks, a workspace admin rotates the key under **Connections > API Keys**.
...
| Way | Address | Notes |
|---|---|---|
| The key in the address | `{{mcpKeyUrl}}` | The route above |
