...
<Tip>
Prefer tool calls to shell commands? Connect over MCP and you get 21 tools with no local install.
They are not identical to the CLI: MCP adds image generation and a device upload panel in some chat apps, and has no post delete, status change or channel analytics. Setup: [MCP setup](/mcp/setup).
</Tip>
...
Your key lives in the app at [app.postqueen.ai](https://app.postqueen.ai): open **Connections > API Keys** and click **Reveal**. Only a workspace admin can reveal the key. Treat it like a password, since it grants full access to your account.
</Warning>
...
<Note>
On the hosted service the CLI signs in with an API key only. `postqueen auth:login` explains where the key is and makes no network call, unless you point it at a device-flow auth server you run yourself with `--auth-server` or `POSTQUEEN_AUTH_SERVER`.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="MCP setup" icon="plug" href="/mcp/setup">
    Connect an MCP client to PostQueen's 21 hosted tools in one line.
  </Card>
</CardGroup>
