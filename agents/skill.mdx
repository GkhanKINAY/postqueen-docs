...
description: 'Skills registry, plus Claude Code, Cursor and Grok plugins from the same repo'
icon: 'download'
---

{/* OVERHAUL-TODO(B6): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file, moved from agents/skill-install (fix; WIP: better, use as base; audit: agents). Row in page-status-2026-09-23.md. */}
...
| Claude Code plugin | `/plugin marketplace add GkhanKINAY/postqueen-agent` | Claude Code only. Versioned, updatable, installable per project |
| Cursor plugin | symlink the repo into `~/.cursor/plugins/local/postqueen` | Cursor. Same skill, no MCP server in the plugin |
| Grok Build plugin | `.grok-plugin/` marketplace in the same repo | Grok Build. Skill and CLI only |

If you are not sure, take the registry route. It is one line, it works from Codex to Cursor to Hermes, and nothing stops you adding a plugin later.
...
## Route 3: Cursor plugin

The repository ships a [Cursor plugin](https://cursor.com/docs/reference/plugins) at `.cursor-plugin/plugin.json` plus a marketplace catalog. It loads the same `postqueen` skill. None of these plugins registers an MCP server.
...
Then restart Cursor or run **Developer: Reload Window**. Install the CLI and set `POSTQUEEN_API_KEY` before you ask the agent to post.

## Route 4: Grok Build plugin

The same repo carries `.grok-plugin/plugin.json` and `.grok-plugin/marketplace.json`, so Grok Build can add it as a marketplace source. Identifier: `postqueen@postqueen-agent`. Like the Claude Code and Cursor plugins it is skill and CLI only.
...
<Steps>
  <Step title="Reveal your key">
    Open [app.postqueen.ai](https://app.postqueen.ai), go to **Connections > API Keys**, and click **Reveal**. Only a workspace admin can reveal the key. Self-hosting? The same screen lives in your own instance.
  </Step>
...
Keep the key secret: it grants full access to your workspace. An admin can rotate it any time under **Connections > API Keys**.
...
| `POSTQUEEN_AUTH_SERVER` | No | none | Only for a device-flow auth server you run yourself. PostQueen runs none |

<Note>
On the hosted service the CLI signs in with an API key only. `postqueen auth:login` makes no network call unless you point it at a device-flow auth server you run yourself: see [Authentication](/cli/introduction).
</Note>
...
More on this in [CLI troubleshooting](/agents/troubleshooting).
...
postqueen posts:create \
  ...
  --settings '{"who_can_reply_post":"everyone","post_type":"post"}' \
