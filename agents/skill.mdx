...
The skill is for agents that run shell commands, such as Claude Code, Codex, Cursor, Gemini CLI, Grok Build, OpenClaw and Hermes Agent. Chat assistants such as Claude, ChatGPT and Grok do not run commands on your machine: connect them over [MCP](/mcp/introduction) instead.
...
- **The CLI where the agent runs:** `npm install -g postqueen`, with Node.js 20.19 or newer (22.12 or newer on Node 22). The skill tells the agent to install it when it is missing, but installing it first saves a step.
