...
{/* OVERHAUL-TODO(B7): write it from _templates/agent.mdx. Sources: main:mcp/clients/grok-build.mdx (merge; WIP: better, use as base; audit: agents), this file (fix; WIP: better, use as base; audit: agents). Row in page-status-2026-09-23.md. */}
...
<Steps>
  <Step title="Connect over MCP">
    One command writes `~/.grok/config.toml`. The scoping options, the header variant and self-hosted URLs are on [Grok Build over MCP](/agents/grok-build).
...
<CardGroup cols={2}>
  <Card title="Grok Build MCP setup" icon="plug" href="/agents/grok-build">
