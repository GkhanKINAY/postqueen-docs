...
{/* OVERHAUL-TODO(B7): write it from _templates/agent.mdx. Sources: this file, moved from mcp/clients/zed (merge; WIP: better, use as base; audit: agents). Row in page-status-2026-09-23.md. */}
...
<Warning>
Your API key can post to every account you have connected. Settings files are plain text. If the key leaks, have an admin rotate it under **Connections > API Keys**, then update every client that uses it.
</Warning>

## Self-hosted

Swap the host for your `NEXT_PUBLIC_BACKEND_URL`. Most self-hosted setups serve the backend under `/api`, so the path sits at `/api/mcp`. The reverse proxy has to forward `/mcp` and stream the response: [MCP on a self-hosted install](/configuration/mcp).
