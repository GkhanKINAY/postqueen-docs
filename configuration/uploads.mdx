...
{/* OVERHAUL-TODO(B9): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: better, use as base; audit: selfhost). Row in page-status-2026-09-23.md. */}
...
| Backend | Where files go | What it asks of you |
|---|---|---|
| `local` | The server's own filesystem, served under `/uploads` | More than one backend replica needs a shared volume, and providers that pull media by URL need that path reachable over public HTTPS |
| `cloudflare` | A Cloudflare R2 bucket, read over public HTTPS | Credentials and a bucket URL, set up in the [R2 guide](/configuration/r2) |
...
| `NEXT_PUBLIC_UPLOAD_STATIC_DIRECTORY` | The URL path segment used to reference those files, **with no leading slash**. Default `uploads` |

<Warning>
`NEXT_PUBLIC_UPLOAD_STATIC_DIRECTORY` is joined after a slash that is
already there, so writing `/uploads` gives you a double slash in every
media URL.
</Warning>
...
| Running from | What serves `/uploads/` |
|---|---|
| The official image | nginx, straight off the container filesystem, from the hard-coded path `/uploads` |
| Source | Next.js rewrites `/uploads/:path*` to its own `/api/uploads/:path*` route handler, which reads `UPLOAD_DIRECTORY` from the frontend process's own disk |
...
<Warning>
Local storage saves each file's full address, `FRONTEND_URL` plus
`/uploads/...`, in the database. Change `FRONTEND_URL` later (a new domain,
`http` to `https`, another port) and the old addresses keep pointing at the old
one: existing media stops loading and can no longer be published. Move them as
described below.
</Warning>

### Moving to a new address

Put the address the instance used to run under in `PREVIOUS_FRONTEND_URLS`,
next to the new `FRONTEND_URL`, and recreate the container.
...
On boot the backend moves every stored upload address under the old one to the
new one: the media library, posts, drafts and channel avatars. The backend log
says what it moved:

```text
[uploads] moved http://localhost:4007 to https://postqueen.example.com: Media.path 12, ...
```

- Moved more than once? List every earlier address, separated by commas.
- Write each one in full, with `http://` or `https://`.
- An address is skipped, with a warning in the log, when none of its newest
  files exist under `UPLOAD_DIRECTORY`. Check that the uploads volume is mounted
  there, then recreate the container again.
- Leaving the variable set afterwards is safe: once moved, nothing matches.

This covers local storage only. Cloudflare R2 addresses come from
`CLOUDFLARE_BUCKET_URL` and are left alone.
...
If your deployment is internet-facing through a reverse proxy with TLS,
you are fine. If PostQueen is on a private network, those providers
will fail and you should use [Cloudflare R2](/configuration/r2) or a
CDN instead.
...
You have three choices for it, a custom domain, the r2.dev URL, or PostQueen
serving the media itself, compared in the
[R2 setup guide](/configuration/r2#decide-where-media-is-read-from). Any of them
gives you a public HTTPS address, so the TikTok caveat above does not apply.
...
| Mode | Setting | Trade |
|---|---|---|
| Straight to the bucket | Unset (the default) | Faster, and it costs you no bandwidth |
| Through the server | `UPLOAD_VIA_SERVER="true"` | Sturdier: nothing between the browser and the bucket can interfere, and it lets the bucket stay private |
...
alters the request between the browser and R2, whether a privacy extension
stripping what it takes for tracking parameters or a corporate proxy, the
signature no longer matches and R2 answers `403`.
...
<Info>
Note the `/api` prefix. nginx inside the container maps `/uploads/` to a local
directory, so only `/api/uploads` reaches the backend.
</Info>

<Warning>
Use a bucket that holds **only** PostQueen media. This endpoint is public by
necessity, since the social networks fetch it with no session, so anything in
that bucket is potentially reachable. It will only serve objects whose names
match what PostQueen itself writes (a random name plus an image, video or audio
...
limits are documented in [troubleshooting/uploads](/troubleshooting/common-errors).

## Next steps

<CardGroup cols={2}>
  <Card title="Set up Cloudflare R2" icon="cloud" href="/configuration/r2">
    The bucket, the token, the CORS policy and the three ways media can be read back
  </Card>
  <Card title="When an upload fails" icon="triangle-alert" href="/troubleshooting/common-errors">
