...
Separate from all of the above, post creation through the public API is capped per hour, and so
are uploads. It defaults to 90 requests per hour and is set with:

```env
API_LIMIT=30
```

Posting from the web interface is not counted. Uploads are, in an hourly allowance of the same
size kept apart from the posting one: `POST /public/v1/upload`, `/public/v1/upload-from-url` and
the uploads you make in the app. See [Public API](/public-api/introduction) for what counts
against it.
