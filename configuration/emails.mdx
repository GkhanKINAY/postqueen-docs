...
{/* OVERHAUL-TODO(B9): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (keep; WIP: same as main; audit: selfhost). Row in page-status-2026-09-23.md. */}

Someone asks for a password reset and the mail is waiting in their inbox. She sends it through
Resend or through your own SMTP server, whichever you point her at.
...
Account activation is a separate choice, made with `REQUIRE_EMAIL_ACTIVATION=true`. Set it and
everyone who signs up with an email and password has to click a link in their mail before they
can sign in. Leave it unset and new accounts are active immediately, provider or not, so password
resets work without anyone waiting on an activation mail.

Activation takes effect only once mail can actually go out: a provider, plus `EMAIL_FROM_ADDRESS`
and `EMAIL_FROM_NAME`. With activation on, that makes the half-configured provider the worst state
to be in: accounts get created inactive, and the mail that would let them in never arrives. Set
the provider **and** its credentials, or leave activation off.
...
<AccordionGroup>
  ...
  <Accordion title="New accounts are stuck as inactive">
    That is activation, switched on with `REQUIRE_EMAIL_ACTIVATION=true`, working while mail is
    not. Fix the provider, then have the person request
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" icon="sliders-horizontal" href="/configuration/reference">
