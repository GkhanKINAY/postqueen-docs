---
title: Email Notifications
description: 'How to send notifications to users'
---

Someone signs up and an activation mail is waiting in their inbox. She sends it through Resend or through your own SMTP server, whichever you point her at.

Configure a provider and the people you invite have to activate before they can sign in.

```env
EMAIL_PROVIDER: "resend|nodemailer"
```

You must also set the sender name and email address for all providers as follows;

```env
EMAIL_FROM_NAME: "PostQueen Emailer"
EMAIL_FROM_ADDRESS: "postqueen@example.com"
```

## Resend

Connect your domain once and she sends from it.

What turns email on is `EMAIL_PROVIDER=resend`, not the key. That is also what
switches account activation on, so everyone you invite from then on has to click
an activation link. Name the provider without finishing the setup and you get the
worst case: accounts are created inactive and the mail that would let them in
fails silently. Set both or set neither.

<Steps>
  <Step title="Register on Resend">
    Register to [Resend](https://resend.com), and connect your domain.
  </Step>
  <Step title="Copy your API Key">
    Copy your API Key from the Resend control panel.
  </Step>
  <Step title="Edit your .env file">
    Open the .env file and edit the following line.

```env
EMAIL_PROVIDER="resend"
RESEND_API_KEY="<your-api-key-here>"
```
  </Step>
</Steps>

## NodeMailer (SMTP)

Already have an SMTP server? Point her at it instead of Resend. NodeMailer is the SMTP library she uses to reach it.

```env
EMAIL_PROVIDER: "nodemailer"
EMAIL_HOST: "smtp.gmail.com" # smtp host if you choose nodemailer
EMAIL_PORT: "465" # smtp port if you choose nodemailer
EMAIL_SECURE: "true" # smtp secure if you choose nodemailer
EMAIL_USER: "user" # smtp user if you choose nodemailer
EMAIL_PASS: "pass" # smtp pass if you choose nodemailer
```

