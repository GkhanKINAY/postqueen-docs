...
a different thing entirely: see [Providers](/channels/overview).
</Note>
...
Your OIDC provider replaces the Google and Apple buttons, and the Farcaster and wallet buttons with them. It does not replace GitHub, because GitHub sign-in and OIDC can never appear together in the first place: GitHub renders only when `IS_GENERAL` is unset, and OIDC renders only when it is set.
