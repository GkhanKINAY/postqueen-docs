...
before you expect it to take effect. The exception is the handful of `NEXT_PUBLIC_`
values consumed inside browser code, which are compiled into the bundle when the image
is built and need a rebuild rather than a restart. Each of those says so where it is
listed.
...
It has a second job that decides how dangerous rotating it is. When `ENCRYPTION_KEY` is unset, this same value also encrypts everything PostQueen stores on your behalf: every channel's access and refresh tokens, provider credentials and saved cookies. Change it in that state and those rows can no longer be decrypted, so every connected channel goes dark and reconnecting each one by hand is the only way back.
...
The key used to encrypt stored channel credentials. Unset, it falls back to `JWT_SECRET`, which is why every install that has never set it has the two jobs welded together.
