...
Per-hour allowance, per organization, on the public-API create-post endpoint. Uploads draw on the same number in a bucket of their own, so a burst of them cannot use up the posting allowance: that covers `POST /public/v1/upload`, `/public/v1/upload-from-url` and the uploads you make in the app. Defaults to `90`, and the shipped `docker-compose.yaml` sets `30`, so raise it there if you drive PostQueen from scripts or upload in bulk. It is one number for the whole instance and the same on every plan: channel and post quotas are what plans tier.
