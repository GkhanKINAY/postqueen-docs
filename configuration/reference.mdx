...
{/* OVERHAUL-TODO(B9): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: better, use as base, but strip the clip…; audit: selfhost). Row in page-status-2026-09-23.md. */}

She reads everything she needs from environment variables. Change one and restart her
before you expect it to take effect.
...
restarting is enough. The single genuine exception is `NEXT_PUBLIC_APP_VERSION`, the version
label she shows, which is baked in when the image is built.
</Note>
...
Redis connection string used for rate limiting and short-lived caches. Scheduling lives in Temporal, not Redis.
...
<Warning>
**Writing `false` turns most of these on.** Only seven variables compare against the literal
string `true`: `DISABLE_REGISTRATION`, `PASSWORDLESS_LOGIN`, `DISABLE_SSRF_PROTECTION`,
`TEMPORAL_TLS`, `EMAIL_SECURE`, `REQUIRE_EMAIL_ACTIVATION` and `UPLOAD_VIA_SERVER`. Every other switch on this page is read as
...
<Note>
One exception is worth knowing before you rely on this. Your own OIDC provider is **exempt**: a person who signs in through it for the first time still gets an account, even with this set. GitHub, Google and Apple are not exempt. So if your goal is to close the door completely, close it at your identity provider as well.
</Note>

### `API_LIMIT`

Per-hour allowance, per organization, on the public-API create-post endpoint. Defaults to `90`, and the shipped `docker-compose.yaml` sets `30`, so raise it there if you drive PostQueen from scripts. It is one number for the whole instance and the same on every plan: channel and post quotas are what plans tier. Uploads have their own allowance, `UPLOAD_LIMIT`.

### `UPLOAD_LIMIT`

Per-hour allowance, per organization, on uploads: `POST /public/v1/upload`, `/public/v1/upload-from-url` and the uploads you make in the app. Defaults to `300`. Before v3.5.2 uploads drew on `API_LIMIT` instead, so with the shipped `30`, choosing more than 30 files at once in the media library failed the rest.

### `RUN_CRON`

When set, the backend process runs the scheduled-task workers. Leave unset on API-only instances when workers are deployed separately. Any non-empty value counts as set, `false` included, so remove the variable to turn it off.
...
The product-line switch, and the shipped compose file turns it on. It decides where `/` lands (`/launches`, the calendar, when set, and `/analytics` when not), which sign-in buttons appear (your own OIDC provider when `POSTQUEEN_GENERIC_OAUTH` is also set, [Google](#sign-in-with-google) otherwise, joined by [Apple](#sign-in-with-apple) once it is configured, and GitHub when `IS_GENERAL` is unset), and a handful of labels and billing screens. Set it to `"true"`. Google sign-in uses the YouTube OAuth client; there is no `GOOGLE_CLIENT_ID`.

<Warning>
Despite the name, `IS_GENERAL` is **not** the switch between a hosted and a self-hosted install. Plans, limits and billing turn on only when both `STRIPE_PUBLISHABLE_KEY` and `STRIPE_SECRET_KEY` are set. Without them every workspace behaves as the top tier, whatever `IS_GENERAL` says.
</Warning>
...
Every request the backend and the orchestrator send to a URL somebody could influence goes through an SSRF guard. That covers calls to a network's API, the self-hosted ones included (WordPress, Mastodon, Lemmy, Listmonk, a Bluesky PDS), the media she downloads before handing it to a network, outgoing webhooks and `/public/v1/upload-from-url`. The guard resolves the host and refuses loopback, private and link-local addresses. A refused request fails as `fetch failed`, with `Error: Blocked IP` in the backend or orchestrator logs.

Set it to `true` to switch the guard off. Both the backend and the orchestrator read it, so set it wherever those two take their environment.

<Warning>
This switches the guard off for the whole instance, not for one provider. Only do it when PostQueen has to reach those services over a trusted private network (the same Docker network, a VPC, a home LAN) **and** everyone with an account on the instance is trusted: anyone who can submit a URL can then make the server fetch your internal services. When you can, fix DNS or routing instead, so the hostname resolves to an address the guard accepts.
</Warning>

### `NOT_SECURED`

Set `true` so login cookies work over plain HTTP (Safari, and hosts that are not treated as
localhost). On Compose, put it in the sibling `.env`; the YAML does not set it for you.

<Warning>
Dev and local HTTP only. Never set in production: it disables security checks that exist for
a reason. Production uses HTTPS instead. See [Try her locally](/installation/quickstart-local)
and [Domain and HTTPS](/installation/domain-and-https).
</Warning>
...
| `PREVIOUS_FRONTEND_URLS` | Addresses a `local`-storage instance used to run under, comma-separated and written in full. On boot, upload addresses stored under them are moved to the current `FRONTEND_URL`. See [Moving to a new address](/configuration/uploads#moving-to-a-new-address). |
...
| `REQUIRE_EMAIL_ACTIVATION` | `true` makes a new email-and-password account wait for its activation link before it can sign in. Off unless set. |

<Warning>
Activation has a switch of its own. A new email-and-password account has to be activated before it can be used only when `REQUIRE_EMAIL_ACTIVATION` is `true` **and** the install can deliver mail, meaning `EMAIL_PROVIDER` names a provider and both `EMAIL_FROM_ADDRESS` and `EMAIL_FROM_NAME` are set. Short of either, the account is active immediately, so configuring a provider for password resets does not switch activation on by itself.

The trap is turning activation on with a provider you have not finished configuring. `EMAIL_PROVIDER=resend` with no `RESEND_API_KEY` still counts as a provider, so every new signup is created inactive, the activation mail fails quietly, and nobody can get in. Either configure the provider fully or leave `REQUIRE_EMAIL_ACTIVATION` unset. Sign-ups through a provider such as Google or your own OIDC provider skip activation, because the provider has already vouched for the address.
</Warning>
...
There is no `GOOGLE_CLIENT_ID`. When `IS_GENERAL` is set and you have not switched on
`POSTQUEEN_GENERIC_OAUTH`, the **Continue with Google** button uses the YouTube OAuth client:

| Variable | Purpose |
|---|---|
| `YOUTUBE_CLIENT_ID` | Web client ID. Sign-in and YouTube connect both send this. |
| `YOUTUBE_CLIENT_SECRET` | Matching secret. |

Login asks only for `userinfo.profile` and `userinfo.email`, and comes back to
`{FRONTEND_URL}/integrations/social/youtube`, the same redirect URI as connecting a YouTube
channel. Create **one** web client and put that redirect on it; do not register a second Google
login app. The walkthrough is [YouTube setup](/installation/providers/youtube).

Unset or empty keys make the button still render (it follows `IS_GENERAL`), then fail when Google
rejects the request. A wrong secret is worse: Google still shows consent (that step only needs the
client id), then `/auth/oauth/GOOGLE/exists` returns 500 with `invalid_client`. Add a new secret on
the same web client and recreate the container; a plain restart keeps the old value.

GitHub sign-in is the other product line: it shows only when `IS_GENERAL` is
unset, and uses `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET`.

---

## Sign in with Apple

An Apple button joins Google on the sign-in and sign-up screens once `APPLE_CLIENT_ID` is set. It
follows Google's rules: it shows only when `IS_GENERAL` is set, and your own OIDC provider
replaces it.

| Variable | Purpose |
|---|---|
| `APPLE_CLIENT_ID` | The Services ID you create for web sign-in in your Apple developer account. Setting it is what makes the button appear. |
| `APPLE_TEAM_ID` | Your Apple developer Team ID. |
| `APPLE_KEY_ID` | The ID of the Sign in with Apple private key. |
| `APPLE_PRIVATE_KEY` | The contents of that `.p8` key. On a single line, write the line breaks as `\n` and she restores them. |
| `APPLE_APP_BUNDLE_ID` | Optional. The bundle ID of a native iOS app of your own, accepted as a second audience so that app can sign people in with Apple too. Leave it unset for the web alone. |

Set the first four together. The button renders on `APPLE_CLIENT_ID` alone, and with any of the
other three missing the sign-in fails.

On the Services ID, register `{NEXT_PUBLIC_BACKEND_URL}/auth/oauth/apple/redirect` as the return
URL, for example `https://postqueen.example.com/api/auth/oauth/apple/redirect`. Apple posts the
result there. With a backend URL that is not https she wraps it through `redirectmeto.com`, the
same way the social networks' callbacks are wrapped, so register the wrapped form in that case.
...
ships with one). The scheduling architecture: [How PostQueen works](/introduction).
...
| `MCP_ONLY` | Strips the backend down to the MCP endpoint and the OAuth routes MCP clients authorize through. Every other route, the web app's and the public API's alike, is left unregistered, so set it only on an instance that exists to serve MCP. Read as a truthy switch: remove the line to turn it off. |
| `DCR_VERIFIED_DOMAINS` | Comma separated domains that clients registering themselves through `POST /oauth/register` may send people back to, which is how MCP clients sign up. With a list, an https redirect address has to be on a listed domain or a subdomain of one. Loopback addresses (`localhost`, `127.0.0.1`, `[::1]`) and app schemes such as `cursor://` are not checked against it, so local clients keep working. Unset, any https address registers. Addresses a browser would execute, such as `javascript:`, are refused either way. |
| `OPENAI_OAUTH_CLIENT_ID` | The OAuth client ID of your ChatGPT connector. That client alone may ask for the `openid` and `email` scopes, and with it set her discovery document advertises `/oauth/userinfo`, which returns the signed-in user's ID, email, whether the email is verified, and name. ChatGPT Enterprise reads this to restrict a connector to the company's email domains. Unset, none of this is offered. |
| `ENTERPRISE_SECRET` | Leave it unset unless you run a reseller integration built against it. It is the key for a handful of provisioning routes under `/enterprise` and for `/public/modify-subscription`, and while it is unset they refuse every request. If you do set it, give it a value of its own, never the one in `JWT_SECRET`. |
...
| `EVOLINK_API_KEY` | EvoLink API key. Setting it turns on the Seedance 2.0 engine in [AI video](/using/media-library#ai-video). |
...
| `STRIPE_SIGNING_KEY` | Stripe webhook signing key for subscription events. Point the webhook at `{NEXT_PUBLIC_BACKEND_URL}/payment/stripe`. The older `{NEXT_PUBLIC_BACKEND_URL}/stripe` runs the same handler, so an endpoint registered there keeps working. |
...
self-hosted install needs none of this, and leaving it unset simply means no billing. Billing switches on only when both `STRIPE_PUBLISHABLE_KEY` and `STRIPE_SECRET_KEY` are set.

### In-app purchases (RevenueCat)

If you publish a mobile app of your own that sells subscriptions through the App Store or Google
Play, she can take its subscription state from RevenueCat. Nothing here is needed for the web app.

| Variable | Purpose |
|---|---|
| `REVENUECAT_WEBHOOK_SECRET` | The value RevenueCat sends in the webhook's `Authorization` header. It has to match exactly, and while it is unset every webhook is refused with `401`. |
| `REVENUECAT_SECRET_KEY` | RevenueCat secret API key. On each webhook she asks RevenueCat for the subscriber's current state instead of trusting the event body, and without this key the webhook answers `400` with `RevenueCat is not configured`. |
| `IN_APP_PURCHASE_REJECT_SANDBOX` | Set it and sandbox purchases are ignored, so a test purchase never grants a plan. Read as a truthy switch. |

Point the RevenueCat webhook at `{NEXT_PUBLIC_BACKEND_URL}/payment/revenuecat`, then check two
things in your app before the first real purchase:

- The app user ID it hands RevenueCat is the PostQueen **organization** ID.
- Every product ID ends in `<tier>.<period>` or `<tier>_<period>`, where the tier is `standard`,
  `team`, `pro` or `ultimate` and the period is `monthly` or `yearly`, for example
  `com.example.app.pro.yearly`. Any other product ID is refused.

An organization is billed on one platform at a time. Once its subscription comes from a store, the
web billing screens answer *Your subscription is managed on mobile*, and cancelling happens in the
store as well. Deleting the PostQueen account does not cancel a store subscription.
...
| `SUPPORT_EMAIL` | Where **Contact support** and **Report a bug** in the Help menu address their mail. Unset on a self-hosted instance (no billing), the Help menu leaves those two rows out rather than mail anyone else; set it to an inbox you read to bring them back. |
| `CHANGELOG_URL` | Adds a **What's new** link to the Help menu. Unset, the entry is not shown. |
| `COMMUNITY_URL` | Adds a **Community** link to the Help menu. Unset, the entry is not shown. |
...
| `NEXT_PUBLIC_APP_VERSION` | The version label shown beside the logo and in the Help menu, and added to the signature of support mail. Baked in when the image is built, and taken from the repository's `package.json` when unset, so set it only when you build your own image and want a different label. |
...
Not every network appears below, and that is the point. Seven of them ask for no
...
Every network in [Providers](/channels/overview) connects from Add Channel whether
...
| LinkedIn | `LINKEDIN_CLIENT_ID`, `LINKEDIN_CLIENT_SECRET` | [/installation/providers/linkedin](/installation/providers/linkedin), [/installation/providers/linkedin-page](/installation/providers/linkedin) |
...
| YouTube (and Google sign-in) | `YOUTUBE_CLIENT_ID`, `YOUTUBE_CLIENT_SECRET` | [/installation/providers/youtube](/installation/providers/youtube), [Sign in with Google](#sign-in-with-google) |
| Google My Business | `GOOGLE_GMB_CLIENT_ID`, `GOOGLE_GMB_CLIENT_SECRET` (optional; falls back to the YouTube client) | [/installation/providers/google-my-business](/installation/providers/google-my-business) |
| TikTok | `TIKTOK_CLIENT_ID`, `TIKTOK_CLIENT_SECRET` | [/installation/providers/tiktok](/installation/providers/tiktok) |
| TikTok Business | `TIKTOK_BUSINESS_CLIENT_ID`, `TIKTOK_BUSINESS_CLIENT_SECRET` | [/installation/providers/tiktok](/installation/providers/tiktok) |
...
| Slack | `SLACK_ID`, `SLACK_SECRET` | [/installation/providers/slack](/installation/providers/slack) |
...
| Apple (sign-in) | `APPLE_CLIENT_ID`, `APPLE_TEAM_ID`, `APPLE_KEY_ID`, `APPLE_PRIVATE_KEY`, `APPLE_APP_BUNDLE_ID` | [Sign in with Apple](#sign-in-with-apple) |
...
### Hiding a provider, or moving its channels to another

Two variables change what Add Channel offers without touching any keys. Both take provider
identifiers, the last part of the connect address, such as `tiktok` in `/integrations/social/tiktok`.

| Variable | Purpose |
|---|---|
| `HIDDEN_PROVIDERS` | Comma separated identifiers to leave out of Add Channel, for example `tiktok,x`. Only the list changes: channels already connected on those providers are left as they are. |
| `MIGRATE_PROVIDERS` | Comma separated `old:new` pairs, for example `tiktok:tiktok-business`. Reconnecting an `old` channel then runs the `new` provider's authorization and moves the channel across in place, keeping its ID, its scheduled posts and its settings. Connecting the same account through `new` from scratch adopts the existing channel rather than adding a duplicate. |

The two are usually set together: hide the provider you are retiring, and existing channels move
over on their next reconnect. A pair whose target is not a provider this install offers is ignored.
Facebook, Instagram through Facebook, LinkedIn Page, YouTube, Google Business Profile and Tumblr
cannot be a target, because they finish their connect differently.
...
[Listmonk](/channels/listmonk) for the channel. There is no Beehiiv channel at
...
The shipped `docker-compose.yaml` and `.env.example` still carry four variables that do nothing. PostQueen does not read `STRIPE_SIGNING_KEY_CONNECT`, `FEE_AMOUNT`, `SLACK_SIGNING_SECRET` or `NX_ADD_PLUGINS` at all, so setting them changes nothing and you can delete the lines.
