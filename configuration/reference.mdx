...
When set, registration refuses any email address containing a `+`. It exists to stop one person opening account after account with plus-addressed aliases. It has no effect on OAuth sign-in, and none on the interface. Leave it unset unless you are running open signups, because plus addressing is something real people use.
...
Every one of these is optional, and a self-hosted install runs perfectly well with none of
them set. Product analytics and error reporting are wired up for the usual vendors, so if you
already pay for one you can point PostQueen at it. The variable names live beside their
neighbours in [`.env.example`](https://github.com/GkhanKINAY/postqueen-app/blob/main/.env.example),
which is the file to copy from when you want one of them on.

Frontend values are read at build time, so a `NEXT_PUBLIC_` change needs a rebuild rather
than a restart.
