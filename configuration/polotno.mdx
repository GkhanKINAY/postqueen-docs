...
{/* OVERHAUL-TODO(B9): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: same as main; audit: selfhost). Row in page-status-2026-09-23.md. */}
...
<Steps>
  ...
  <Step title="Restart PostQueen">
    ...
    Running from source, restart the dev server.
  </Step>
</Steps>
...
Leave the variable unset and a self-hosted instance still shows **Design
Media** and **Editor**. Clicking one explains which variable to set, to admins,
or asks members to contact an admin, instead of opening an editor without a
key. PostQueen Cloud shows the editor only where it is configured, so you never see that note there.
