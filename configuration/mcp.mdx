...
{/* OVERHAUL-TODO(B9): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file, moved from mcp/self-hosting (fix; WIP: partly better; audit: agents). Row in page-status-2026-09-23.md. */}
...
<Tabs>
  ...
  <Tab title="Caddy">
    ...
    Caddyfile on the [Caddy](/installation/domain-and-https) page already works for MCP.
  </Tab>
...
MCP works without any of these. Each one changes a single thing:

| Variable | What it changes |
| --- | --- |
| `DCR_VERIFIED_DOMAINS` | Limits which https addresses a self-registering client may send people back to. Clients on `localhost` or an app scheme such as `cursor://` are not affected |
| `OPENAI_OAUTH_CLIENT_ID` | Lets one ChatGPT connector read the signed-in user's email, which ChatGPT Enterprise needs to restrict a connector by domain |
| `MCP_ONLY` | Strips the backend down to MCP and its OAuth routes, for an instance that serves nothing else |

Each is described in full in the [Configuration Reference](/configuration/reference).
...
<CardGroup cols={2}>
  ...
  <Card title="Limits and troubleshooting" icon="circle-minus" href="/agents/troubleshooting">
