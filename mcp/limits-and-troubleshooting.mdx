...
sidebarTitle: Limits
description: 'What the 14 MCP tools cannot reach, and what to do when the connection misbehaves'
icon: 'gauge'
---

The tools she answers over MCP are built to make things: they read your channels and their rules, then create posts and media. They can also list what is on the calendar and change a scheduled post's settings. Deleting, rescheduling, analytics and anything that touches a connection live on the [Public API](/public-api/introduction) and the [CLI](/cli/introduction) instead.

<Info>
**No MCP tool deletes a post, moves one to a new time, or returns analytics.** Ask your agent to
...
Regenerating the key in **Settings → API Keys** takes the old one out immediately, and every client still holding it stops mid-conversation. PostQueen keys do not rotate on their own, so this only happens when you ask for it.
...
<Warning>
Prefer `/mcp` or `/mcp/YOUR_API_KEY` over streamable HTTP. `/mcp-oauth*` is for directory OAuth
clients and exposes a shorter tool set (see [Tools Reference](/mcp/tools#how-many-tools-you-actually-get)).
The SSE pair is still mounted and still serves the full 14-tool set, but it is the older
...
<AccordionGroup>
  ...
  <Accordion title="Can MCP read or reply to comments?">
    ...
    media: there is no `getComments` or `replyToComment` tool. The closest feature is
