...
| Move a post between draft and schedule | `schedulePostTool` sets `type` when the post is created and never again | [`PUT /posts/{id}/status`](/public-api/posts/change-status), or `postqueen posts:status <id> --status schedule` |
...
`schedulePostTool` accepts `draft`, `schedule` or `now` as the `type`.
