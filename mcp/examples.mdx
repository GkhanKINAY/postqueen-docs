---
title: Examples
description: 'Common workflows when using PostQueen through MCP'
---

You never type these. You say what you want in plain language, your agent makes the calls, and she puts the result on your calendar.
...
<Steps>
  ...
  <Step title="Schedule the post">
    The agent calls `integrationSchedulePostTool`:
...
Instagram (Facebook Business-linked) offers an `audioSearch` tool for finding
...
<Note>
Audio is not available on `instagram-standalone` channels, because the Instagram
Audio API requires Facebook Login.
</Note>
...
<Steps>
  <Step title="Generate the image">
    ...
    {
      "id": "img-456",
      "path": "https://uploads.postqueen.ai/generated-image.png"
    }
    ```
  </Step>
      ...
      "postsAndComments": [
        {
          ...
          "attachments": ["https://uploads.postqueen.ai/generated-image.png"]
        }
      ]
    }
    ```
...
<Steps>
  ...
  <Step title="Schedule with the video">
    The agent uses the video URL as an attachment when calling `integrationSchedulePostTool`.
  </Step>
</Steps>
