...
Point a client at `/mcp` (or `/mcp/:apiKey`) and your agent gets **14 tools** it can call. Other MCP paths expose a shorter set. It reads their schemas, then calls them on your behalf.
...
| `integrationSchedulePostTool` | Schedule, draft, or immediately publish posts. This is the MCP map key. The Mastra `id` is `schedulePostTool`; clients and error text use `integrationSchedulePostTool` |
| `postsListTool` | List the posts scheduled between two dates, with their current settings |
| `postSettingsTool` | Change the settings of a scheduled post or draft that has not gone out yet |
...
| `generateVideoTool` | Start generating a video for a post |
| `videoStatusTool` | Check whether that video is ready, and get its URL |
...
<Note>
Thirteen of those are the same tools the in-app assistant runs. The fourteenth, `ask_postqueen`, exists
only on the direct MCP servers: your client is talking to her as an agent, so it can either drive the individual
tools itself or hand the whole job to her. That is why pages about the assistant say thirteen
and pages about MCP say fourteen.
</Note>
...
<CardGroup cols={2}>
  <Card title="API key" icon="key">
    Get your API key from **Settings → API Keys**. Use it directly in
...
MCP OAuth itself uses the scopes **`mcp:read`** and **`mcp:write`**. The protected-resource URLs are `/mcp-oauth`, `/mcp-oauth-claude` and `/mcp-oauth-dynamic`. They share one authorization server at `/mcp-oauth`. When `OPENAI_OAUTH_CLIENT_ID` is set, the same server also advertises `openid` and `email`.
...
<AccordionGroup>
  ...
  <Accordion title="What happens when my API key expires or is rotated?">
    PostQueen API keys do not auto-rotate, but if you regenerate one in Settings → API Keys, every MCP client using the old key stops working until you update its config.
