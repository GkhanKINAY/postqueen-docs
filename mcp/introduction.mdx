...
That means you can connect Claude, Cursor, or any MCP-compatible client to your PostQueen account and ask her to handle your social media in plain language.
...
Not today. The current tool set is read-only on integrations and write-only on posts and media: there is no `getComments` or `replyToComment` exposed via MCP. PostQueen does not read incoming comments at all. The closest feature is Auto Actions, which can repost your own post or add a follow-up comment on it once it crosses the like milestone you set.
