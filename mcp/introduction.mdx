...
MCP (Model Context Protocol) lets AI agents interact with Postiz directly, listing integrations, scheduling posts, generating images and videos, clipping long videos into shorts, all through a standardized tool-calling interface.
...
Postiz exposes an MCP server that provides **13 tools** to AI agents. The agent discovers these tools, understands their schemas, and calls them on your behalf.
...
| `clippingTool` | Turn a long YouTube video into captioned vertical clips and draft posts |
| `clippingStatusTool` | Check a clipping and get the finished clips |
...
2. **Agent calls `integrationSchema`** with `platform: "x"` and learns X's character limits, settings, and rules
...
The MCP server starts as part of the Postiz backend and is reachable at `/mcp` (Bearer auth), `/mcp/:apiKey` (key in URL), and `/mcp-oauth` (OAuth-protected). Your reverse proxy must forward these paths to the backend and support streaming HTTP (`Transfer-Encoding: chunked`). See [Reverse Proxies](/self-host/reverse-proxies/caddy).
