...
Your backend already serves it. The endpoint sits at `/mcp` (Bearer auth) and `/mcp/:apiKey` (key in URL). Your reverse proxy has to forward these paths and stream the response rather than buffering it: [MCP on a self-hosted install](/mcp/self-hosting).
