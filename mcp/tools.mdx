...
Thirteen of those are the same tools the in-app assistant runs. The fourteenth, `ask_postqueen`, exists only on the direct MCP servers: your client is talking to her as an agent, so it can either drive the individual tools itself or hand the whole job to her. That is why pages about the assistant say thirteen and pages about MCP say fourteen.
...
|---|---|---|
| `/mcp` and `/mcp/:apiKey` | **14** | Nothing. Thirteen tools plus `ask_postqueen` |
| `/sse/:id` and `/message/:id` | **14** | Nothing. Same full set as `/mcp`. Legacy [SSE transport](/mcp/limits-and-troubleshooting#transports-you-may-notice-and-should-not-build-on); prefer streamable HTTP |
| `/mcp-oauth` and `/mcp-oauth-dynamic` | **13** | No `ask_postqueen`. Directory reviews reject an annotation-less catch-all agent |
| `/mcp-oauth-claude` | **8** | No `ask_postqueen`, and no `generateImageTool`, `generateVideoTool`, `videoStatusTool`, `generateVideoOptions`, or `videoFunctionTool` |

Custom key-in-URL Claude or ChatGPT connectors that hit `/mcp/:apiKey` still see 14. The shorter lists apply to the OAuth directory endpoints, not to every Claude or ChatGPT setup.
