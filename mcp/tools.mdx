...
These are the seventeen tools she answers over MCP, with every parameter and every field she sends back. Your agent reads these schemas on connect, so you never call one by hand.

Sixteen of those are the same tools the in-app assistant runs. The seventeenth, `ask_postqueen`, exists only on the direct MCP servers: your client is talking to her as an agent, so it can either drive the individual tools itself or hand the whole job to her. That is why pages about the assistant say sixteen and pages about MCP say seventeen.

## How many tools you actually get

The count depends on which URL you connected. Direct `/mcp` keeps the full set. Directory OAuth paths drop the agent, and Claude's directory path also hides five media and video tools.

| Path | Tools | What is missing |
|---|---|---|
| `/mcp` and `/mcp/:apiKey` | **17** | Nothing. Sixteen tools plus `ask_postqueen` |
| `/sse/:id` and `/message/:id` | **17** | Nothing. Same full set as `/mcp`. Legacy [SSE transport](/mcp/limits-and-troubleshooting#transports-you-may-notice-and-should-not-build-on); prefer streamable HTTP |
| `/mcp-oauth` and `/mcp-oauth-dynamic` | **16** | No `ask_postqueen`. Directory reviews reject an annotation-less catch-all agent |
| `/mcp-oauth-claude` | **11** | No `ask_postqueen`, and no `generateImageTool`, `generateVideoTool`, `videoStatusTool`, `generateVideoOptions`, or `videoFunctionTool` |

Custom key-in-URL Claude or ChatGPT setups that hit `/mcp/:apiKey` still see 17. The shorter lists apply to the OAuth directory endpoints, which are not how you connect today: PostQueen is not listed in Anthropic's Connectors Directory or the ChatGPT app store.
...
The name your client sees, and the name in validation errors ("try `integrationSchedulePostTool` again"), is **`integrationSchedulePostTool`**. That is the MCP map key (`name` on the tool class). The Mastra `id` is `schedulePostTool`. Call the wire name, not the Mastra id.
...
The seventeenth tool is not one of the sixteen above and does not look like them. Your client will discover it anyway, so it is worth knowing what it is.
