...
These are the 10 tools she answers over MCP, with every parameter and every field she sends back. Your agent reads these schemas on connect, so you never call one by hand.
...
## What these ten tools cannot do

Some jobs have no tool here and need the CLI or the API alongside. Reading analytics is one. Listing or deleting an existing post is another. [Limits and Troubleshooting](/mcp/limits-and-troubleshooting) has the full list, the route to use instead of each gap, and what to check when your client says it cannot reach her at all.
