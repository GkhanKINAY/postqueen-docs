...
<Warning>
Your API key grants full access to your PostQueen workspace: treat it like a password. `~/.grok/config.toml` is plain text. If the key leaks, have an admin rotate it under **Connections > API Keys**, then update every client that uses it.
</Warning>
