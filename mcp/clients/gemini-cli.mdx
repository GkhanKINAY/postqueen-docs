...
<Note>
Keys per the official [Gemini CLI MCP documentation](https://github.com/google-gemini/gemini-cli/blob/main/docs/tools/mcp-server.md): `httpUrl` is the streamable HTTP endpoint (`url` is reserved for SSE servers, and PostQueen does not use SSE). If an older `gemini mcp add` wrote `"url"` plus a type field, change that key to `httpUrl` or the connection fails. A project-level `.gemini/settings.json` uses the same format.
</Note>
