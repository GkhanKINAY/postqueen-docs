...
Gemini CLI supports MCP servers over streamable HTTP through its `settings.json` configuration, so her 14 tools land in the terminal you already have open.
...
It lists configured servers with their connection status and discovered tools. `postqueen` should show as connected with 14 tools. Then ask:
...
<CardGroup cols={2}>
  <Card title="Tools reference" icon="screwdriver-wrench" href="/mcp/tools">
    Every parameter of the 14 tools Gemini just discovered
  </Card>
