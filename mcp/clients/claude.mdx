...
<Warning>
Your API key can post to every account you have connected. Treat it like a password: anyone who sees your connector URL or config file can publish as you. Do not screenshot it and do not commit it. If the key leaks, regenerate it in **Settings → API Keys** and update every client that uses it.
</Warning>

## Option A: custom connector (recommended)

<Warning>
PostQueen is not listed in Anthropic's Connectors Directory. **Browse connectors** will not find her. Use **Add custom connector** below.
</Warning>
...
<Steps>
  <Step title="Open connector settings">
    In Claude Desktop or on claude.ai, open **Customize → Connectors**. Claude Desktop may still label this **Settings → Connectors**.
  </Step>
  ...
  <Step title="Confirm">
    Click **Add**. PostQueen appears in your connector list. Enable it in a chat from **+ → Connectors**. Her 14 tools become available in that conversation.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Tools reference" icon="screwdriver-wrench" href="/mcp/tools">
    Every parameter of the 14 tools Claude just discovered
  </Card>
  ...
  <Card title="Claude Code" icon="/images/brand/icon-claude-code.svg" href="/mcp/clients/claude-code">
    Terminal and IDE agent. Different product: `claude mcp add`, not this connector
  </Card>
</CardGroup>
