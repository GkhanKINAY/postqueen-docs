...
<Info>
Grab your API key from [app.postqueen.ai](https://app.postqueen.ai): open **Connections > API Keys** and click **Reveal**. Only a workspace admin can reveal the key.
</Info>

<Warning>
Your API key can post to every account you have connected. Treat it like a password: anyone who sees your MCP URL or config file can publish as you. Do not screenshot it and do not commit it. If the key leaks, have an admin rotate it under **Connections > API Keys**, then update every client that uses it.
</Warning>
...
<Steps>
  ...
  <Step title="Confirm">
    Click **Add**. PostQueen appears in your list. Enable it in a chat from **+ → Connectors**. PostQueen's 21 tools become available in that conversation.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Tools reference" icon="screwdriver-wrench" href="/mcp/tools">
    Every parameter of the 21 tools Claude just discovered
  </Card>
