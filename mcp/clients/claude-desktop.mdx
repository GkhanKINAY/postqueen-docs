...
Your reverse proxy has to forward the `/mcp` paths and stream the response rather than buffering it. The configuration, and the Nginx setting that silently breaks this, are on [MCP on a self-hosted install](/mcp/self-hosting). For Option A the server must also be reachable from the public internet; if it is not, Option B works because the bridge runs on your machine.
