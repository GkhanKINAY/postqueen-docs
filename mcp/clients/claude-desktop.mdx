...
If you prefer the config file, or your self-hosted server is not publicly reachable, run the [`mcp-remote`](https://www.npmjs.com/package/mcp-remote) bridge locally. It launches on your machine and forwards Claude Desktop's stdio traffic to PostQueen over HTTP. Requires [Node.js](https://nodejs.org/).
...
Your reverse proxy must forward the `/mcp` paths to the backend and support streaming HTTP. Proxy configuration: [Reverse Proxies](/reverse-proxies/caddy). For Option A the server must also be reachable from the public internet; if it is not, Option B works because the bridge runs on your machine.
