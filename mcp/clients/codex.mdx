---
title: OpenAI Codex
description: 'Connect the OpenAI Codex CLI to PostQueen over MCP'
icon: /images/brand/icon-codex.svg
---

The OpenAI Codex CLI supports remote streamable HTTP MCP servers natively, so it talks to her directly, with no local bridge process. This is Codex, not ChatGPT — same pairing as Claude Code vs Claude. ChatGPT uses [Settings → Apps](/mcp/clients/chatgpt). Servers are registered in `~/.codex/config.toml` under `[mcp_servers.NAME]`, either by editing the file or with the `codex mcp add` command.
...
Replace `YOUR_API_KEY` with your actual key. Codex writes the entry to `~/.codex/config.toml`, and all 21 PostQueen tools are available in every session from then on.
...
<CardGroup cols={2}>
  <Card title="Tools reference" icon="screwdriver-wrench" href="/mcp/tools">
    Every parameter of the 21 tools your agent just discovered
  </Card>
