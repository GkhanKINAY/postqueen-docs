...
ChatGPT can talk to PostQueen through a custom MCP connector: once it is set up, you can ask ChatGPT to list your connected accounts, generate images and schedule posts. It calls her [10 MCP tools](/mcp/tools) behind the scenes.
...
Most self-hosted setups serve the backend under `/api`, so the MCP path sits at `/api/mcp`. Your reverse proxy must forward the `/mcp` paths and support streaming HTTP. Proxy configuration: [Reverse Proxies](/reverse-proxies/caddy).
