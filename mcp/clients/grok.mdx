...
Grok Bot uses the **same** connector. There is no Bot-only MCP URL: create the connector first, then enable it on the Bot under Plugins.
...
<Note>
On Grok Business and Enterprise, an admin may need to provision the connector before members can use it. Availability and menu names shift; if you do not see Custom, check [xAI's connectors docs](https://docs.x.ai/grok/connectors).
</Note>
