...
Cursor speaks MCP over streamable HTTP, so PostQueen's server plugs in with a single URL. Once connected, Cursor's agent can list your channels, generate media, schedule posts and read how published posts did without leaving the editor. Channel analytics such as follower numbers stay with the CLI or the API.
...
<Steps>
  ...
  <Step title="Enable the server">
    Back in **Customize → MCP** or **Tools & MCP**, make sure the `postqueen` server is enabled. Cursor connects and lists the server's 21 tools.
  </Step>
</Steps>
