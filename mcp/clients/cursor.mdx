...
Cursor's source of truth is an `mcp.json` file. The Settings UI and the older Customize page both write that file.

<Steps>
  <Step title="Open MCP settings">
    Open **Cursor Settings → MCP**. The **Customize → MCP** page in the sidebar writes the same file if you still have it.
  </Step>
  ...
  <Step title="Enable the server">
    Back on the **Customize** page, make sure the `postqueen` server is enabled. Cursor connects and lists the server's 14 tools.
  </Step>
</Steps>
...
Your channel names come back, because the agent reached for her `integrationList` tool. If it fails, check that the `postqueen` server is enabled under **Cursor Settings → MCP**, that `mcp.json` saved with a `"url"` field, that the API key was pasted in full, and that no firewall blocks `api.postqueen.ai`.
