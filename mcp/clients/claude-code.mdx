...
Claude Code connects to her over streamable HTTP with a single `claude mcp add` command, and her 10 tools are there in the terminal you already have open.
...
Replace `YOUR_API_KEY` with your actual key. By default this registers the server for you in the current project. Add `--scope user` to make PostQueen available in every project on your machine, or `--scope project` to share it with the repo through a checked-in `.mcp.json`.
