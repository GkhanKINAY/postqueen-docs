---
title: Claude Code
description: 'Connect Claude Code to PostQueen over MCP'
icon: /images/brand/icon-claude-code.svg
---

Claude Code is Anthropic's terminal and IDE agent. It is not claude.ai or Claude Desktop — that
chat product uses a [custom connector](/mcp/clients/claude). Same pairing as Codex vs ChatGPT.

Claude Code connects to PostQueen over streamable HTTP with a single `claude mcp add` command, and PostQueen's 21 tools are there in the terminal you already have open.
...
<Warning>
Your API key grants full access to your PostQueen workspace: treat it like a password, and use the `${POSTQUEEN_API_KEY}` form above in any `.mcp.json` you check in. If the key leaks, have an admin rotate it under **Connections > API Keys**, then update every client that uses it.
</Warning>
...
<Tip>
The postqueen skill teaches Claude Code the `postqueen` CLI as well, which reaches what MCP has no tool for: channel analytics, deleting posts, moving a draft to scheduled and uploading a local file. Both routes can run side by side: see [Claude Code as an agent](/agents/claude-code).
</Tip>
...
<CardGroup cols={2}>
  <Card title="Tools reference" icon="screwdriver-wrench" href="/mcp/tools">
    Every parameter of the 21 tools Claude Code just discovered
  </Card>
