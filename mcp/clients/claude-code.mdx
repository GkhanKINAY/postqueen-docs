...
Claude Code connects to her over streamable HTTP with a single `claude mcp add` command, and her 14 tools are there in the terminal you already have open.
...
<Warning>
Your API key grants full access to your PostQueen workspace: treat it like a password, and use the `${POSTQUEEN_API_KEY}` form above in any `.mcp.json` you check in. If the key leaks, regenerate it in **Settings → API Keys** and update every client that uses it.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Tools reference" icon="screwdriver-wrench" href="/mcp/tools">
    Every parameter of the 14 tools Claude Code just discovered
  </Card>
