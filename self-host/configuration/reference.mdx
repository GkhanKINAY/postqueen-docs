...
| `RUNPOD_API_KEY` | RunPod API key with access to the clipping endpoints. See [Video clipping](/self-host/configuration/clipping). |
| `RUNPOD_INGEST_ENDPOINT_ID` | RunPod endpoint (CPU) that runs the ingest jobs of video clipping. |
| `RUNPOD_CLIPPER_ENDPOINT_ID` | RunPod endpoint (GPU) that renders the clips of video clipping. |
| `DEEPGRAM_API_KEY` | Deepgram key that transcribes videos without usable captions for video clipping. |
