...
description: 'Set up PostQueen for local development'
---

Running her from source takes Node, Postgres, Redis and Temporal on your own machine. Only looking to self-host?
[Docker-Compose](/installation/docker-compose) is the shorter road, and it now includes the Temporal stack for workflow processing.

<Warning>
  <strong>Important:</strong> PostQueen uses Temporal for background workflows. Use the maintained Docker Compose repository which includes the Temporal stack: [/installation/docker-compose](/installation/docker-compose).
</Warning>
...
Several users using Windows (and WSL) have reported issues with the setup. This is not well tested as the main developers of the project do not use Windows/WSL for development. Windows and WSL are not officially supported, so help with problems specific to those environments may be limited.
...
- **PostgreSQL** - or any other SQL database (Docker is the easiest route)
- **Redis** - for worker queues (Docker is the easiest route)
- **Temporal** - runs as a separate stack (Postgres + Elasticsearch + Temporal services). For local development run the Temporal stack via the `postqueen-docker-compose` repository described in [/installation/docker-compose](/installation/docker-compose). Set `TEMPORAL_ADDRESS` in your `.env` to point at the Temporal service (example below).
...
You can [view `docker-compose.dev.yaml` on GitHub](https://github.com/GkhanKINAY/postqueen-app/blob/main/docker-compose.dev.yaml)
before you run it, or just take it from the clone in the next step.

```bash
docker compose -f "docker-compose.dev.yaml" up
```

To run Temporal locally, clone the maintained Docker Compose repository which includes the Temporal stack and follow the instructions in [/installation/docker-compose](/installation/docker-compose).
...
```bash
# Required Settings
# This DATABASE_URL matches Option B, the docker-compose.dev.yaml above.
# On Option A the single container was created with POSTGRES_USER=root and the
# password you chose, and with no postqueen-db-local database, so adjust the
# user, the password and the database name to match what you actually started.
...
# Set this explicitly. Uploads are written either way, but the route that serves
# them tests for the literal string "local", so leaving it unset makes every
# uploaded image 404.
STORAGE_PROVIDER="local"

# Optional. Your upload directory path if you host your files locally.
UPLOAD_DIRECTORY="/opt/postqueen/uploads/"
...
CLOUDFLARE_BUCKETNAME="postqueen"
