...
import InstallationRecommendedOptions from '/snippets/installation-recommended-options.mdx';
import NextStepsSelfhost from '/snippets/next-steps-selfhost.mdx';

{/* OVERHAUL-TODO(B9): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: stale; audit: selfhost). Row in page-status-2026-09-23.md. */}
...
Use chart 1.1.3 or newer. Chart 1.1.0 bundles Bitnami PostgreSQL and Redis images that are no
longer published, so its pods cannot pull them; 1.1.3 takes them from `bitnamilegacy/`.
...
<AccordionGroup>
  <Accordion title="The image tag needs its v prefix">
    Published tags look like `v3.5.9`. When `image.tag` is empty the chart falls back to its
