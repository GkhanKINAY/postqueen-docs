...
description: 'Register the Google My Business application and give PostQueen its keys'
...
```
    ...
    <Note>
    PostQueen reads `GOOGLE_GMB_CLIENT_ID` / `GOOGLE_GMB_CLIENT_SECRET` when they are set, otherwise it falls back to `YOUTUBE_CLIENT_ID` / `YOUTUBE_CLIENT_SECRET`. Sharing the YouTube web client is fine: add `{FRONTEND_URL}/integrations/social/gmb` to that client's redirect URIs and add `https://www.googleapis.com/auth/business.manage` on Data Access. You still need the Business Profile APIs enabled and Google's GMB access approval. A second OAuth client is optional, not required.
    </Note>
  </Step>
...
<CardGroup cols={2}>
  <Card title="What she can post to Google My Business" icon="/images/channels/google-business.svg" href="/channels/google-my-business">
