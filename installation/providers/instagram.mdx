...
<Warning>
**NOTE:** Please be advised that Instagram and Facebook can use the same app (no need to create two separate apps). Request the **union** of both providers' permissions on that shared app. Instagram Standalone is a separate app with its own four `instagram_business_*` scopes. Threads is a third app.
</Warning>
...
The permissions are not the ones listed above. Instagram Standalone is a **separate** Meta app (Instagram Login), not the Facebook + Instagram Business app. Meta does not allow Facebook Login and Instagram Login on the same app. Request these four here, and do not mix them into the app above:
