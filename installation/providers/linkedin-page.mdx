...
<Steps>
  ...
  <Step title="Check the granted scopes">
    Open the **Auth** tab of your app. Its **OAuth 2.0 scopes** list must show all seven scopes PostQueen asks for:
    ...
    The last four come from the Advertising API, `w_member_social` from Share on LinkedIn, and `openid` with `profile` from Sign in with LinkedIn.

    <Warning>
    A missing scope does not fail the LinkedIn login. LinkedIn hands back a token without it, and PostQueen then refuses the connection with *Not enough scopes, when choosing a provider, please add all the scopes*. When you see that message, this list is the first place to look.
    </Warning>
  </Step>
