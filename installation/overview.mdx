...
{/* OVERHAUL-TODO(B9): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (fix; WIP: partly better; audit: selfhost), main:installation/system-requirements.mdx (merge; WIP: better, use as base for the outbound-ho…; audit: selfhost). Row in page-status-2026-09-23.md. */}
...
<CardGroup cols={2}>
  ...
  <Card title="I already run Kubernetes" icon="ship-wheel" href="/installation/kubernetes-helm">
...
| [Docker](/installation/docker-compose) | The PostQueen container on its own, with no database or scheduler. You bring your own. | You already run PostgreSQL, Redis and Temporal somewhere, and only want the app. |
...
| [Dev Container](/installation/development) | The development environment above, prebuilt inside a container, so your editor sets it up for you. | You want to contribute without installing Node and pnpm yourself. |

## What she needs to run

A small virtual machine is enough. Two vCPUs and **4 GB of RAM** is the floor for Compose:
Temporal plus Elasticsearch will OOM a 2 GB box. The full numbers, including disk and
the outbound addresses she needs to reach, are on
[System Requirements](/installation/overview).
