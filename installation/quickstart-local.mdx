...
exactly where you left off. This table is for a machine that runs only this stack. On a shared
host, never run a bare `docker compose down`. Recreate one service instead: see
[Restart](/installation/docker-compose#applying-a-change).
