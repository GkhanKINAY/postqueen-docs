...
This guide takes you from nothing to a working PostQueen on your own domain. It assumes no
prior server experience. Every command is written out, and every value you need to change is
shown before and after.

Set aside about thirty minutes. Most of that is waiting for downloads.
...
1. **A small server.** Any provider that rents Linux virtual machines works. Pick Ubuntu 24.04.
   Two vCPUs and 4 GB of RAM is a comfortable starting point. 2 GB will run, but it is tight
   once the scheduler is busy.
2. **A domain name**, or a subdomain of one you already own. She needs a real name to get an
   HTTPS certificate, so an IP address alone is not enough.
3. **A terminal.** macOS and Linux have one built in. On Windows use PowerShell or
   Windows Terminal.
...
<Steps>
  ...
  <Step title="Point your domain at it">
    ...
    When that prints your server's IP address, you are ready. Do this now, because the
    certificate step later will fail if the name does not resolve yet.
  </Step>
  ...
  <Step title="Close the doors you are not using">
    ...
    Notice that port 4007 is **not** on that list, and should not be. PostQueen listens on 4007
    locally, and the reverse proxy you set up later is the only thing that talks to her. The
    outside world only ever sees 443.
  </Step>
...
She is live on your domain, over HTTPS, and closed to strangers. Three things are worth doing
