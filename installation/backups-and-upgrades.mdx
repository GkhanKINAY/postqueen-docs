...
<Warning>
This is one of the few times you stop the whole Compose project. On a machine that runs only PostQueen, `docker compose down` is fine. On a shared host, stop only her services by name. Never run a bare `docker compose down` on a box that also runs other Compose projects.
</Warning>
...
Plain `docker compose down` leaves every volume where it is. That is the command
for a backup window, not for a variable change. Env changes recreate one
service: see [Applying a change](/installation/docker-compose#applying-a-change).
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Configure uploads" icon="cloud-upload" href="/configuration/r2">
    Move media to Cloudflare R2 and shrink what you back up
  </Card>
  <Card title="System requirements" icon="server" href="/installation/overview">
