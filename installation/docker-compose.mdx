...
- **Option B** - a `.env` file next to your `docker-compose.yml`. Compose reads it for `${VAR}` substitution **inside the YAML only**; it does not hand those variables to the container. To feed the container from a file you have to add an `env_file:` key to the service yourself, which [Docker Configuration](/configuration/docker) shows.
