...
{/* OVERHAUL-TODO(B9): rewrite it to the style guide in CONTRIBUTING.md. Sources: this file (keep; WIP: same as main; audit: selfhost), main:reverse-proxies/caddy.mdx (fix; WIP: better, use as base; audit: selfhost). Row in page-status-2026-09-23.md. */}
...
<CardGroup cols={2}>
  <Card title="Caddy" icon="feather" href="/installation/domain-and-https">
  ...
  <Card title="Traefik" icon="network" href="/reverse-proxies/traefik">
  ...
  <Card title="Something else" icon="circle-question-mark" href="/reverse-proxies/split-deployments">
...
| [Docker on its own](/installation/docker-compose) | `localhost:5000` |
...
she builds sign-in origins and every social network's return address from it.

<Snippet file="compose-public-urls.mdx" />
...
If you followed [Deploy to a server](/installation/production), these three URLs are already
in `.env`. Apply a change with a recreate, not a whole-stack teardown:
