...
<AccordionGroup>
  ...
  <Accordion title="A large upload fails">
    Caddy sets no size limit, so look at what else is in the path. A CDN or another proxy in front of Caddy often caps the request body, for example at 100 MB. PostQueen itself accepts files up to 1 GB each.
  </Accordion>
</AccordionGroup>
