...
**Where:** **Channels** > pick a channel > **Automations**. **Open Auto-Plugs** in that section shows every eligible channel on one page.
...
<Steps>
  ...
  <Step title="Choose Set up plug">
    On the row of the action you want. A window titled with the action's name opens.
  </Step>
  ...
  <Step title="Choose Activate">
    "Plug updated" confirms it. The plug is on, and every post this channel publishes from now on is watched.
  </Step>
</Steps>
...
- The post itself still goes out at the time you scheduled. A plug only acts afterwards.

<Warning>
On Bluesky, both actions finish after the first check, about 6 hours after the post publishes. If the post has not reached your number by then, the action does not run for that post. Set a number a good post reaches within a few hours.
</Warning>
...
| "No plugs for these channels" on the Auto-Plugs page | None of your channels is Bluesky, X, Threads or a LinkedIn Page. Connect one of those. |
...
<CardGroup cols={2}>
  ...
  <Card title="Analytics" icon="chart-no-axes-column" href="/using/analytics">
