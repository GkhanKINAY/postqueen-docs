...
**Where:** **Settings** > **Webhooks**, in the More group. The rail's **More** group links there too.

A webhook sends a `POST` request to a URL you own each time a post goes live. Use it to start a workflow in Zapier, Make or n8n, or to update a sheet, a chat channel or your own system.

## Add a webhook

<Steps>
  <Step title="Choose Add a webhook">
    The pane's title counts your webhooks against your plan, for example **Webhooks (1/10)**. You need at least one connected channel first.
  </Step>
  ...
  <Step title="Pick the Channels">
    **All channels** fires for posts on every connected channel. **Specific channels** fires only for the channels you tick.
  </Step>
  <Step title="Choose Send Test">
    PostQueen sends a sample payload to the URL and tells you whether your endpoint accepted it. Set up the receiving side before you save.
  </Step>
  <Step title="Choose Save">
    "Webhook added successfully" confirms it. Each saved webhook shows its name and URL, a **Send Test** button, and buttons to edit or delete it.
  </Step>
</Steps>
...
At the limit, **Add a webhook** is switched off and the pane says "Your plan is at its webhook limit. Remove one or upgrade to add another." See [Plans](/cloud/plans).
...
<CardGroup cols={2}>
  <Card title="Zapier and Make" icon="workflow" href="/public-api/zapier-and-make">
