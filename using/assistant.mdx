...
<Note>
**Self-hosting and cannot find the sparkle button?** Nothing is broken. That button appears only when your plan includes AI **and** billing is switched on, and billing is switched on only when both `STRIPE_PUBLISHABLE_KEY` and `STRIPE_SECRET_KEY` are set. An install with no Stripe keys is unlimited in every other way, yet the button stays hidden. The chat in the Agent tab is not affected and works with no Stripe keys at all.
</Note>
