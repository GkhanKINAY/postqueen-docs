...
PostQueen keeps three small pieces of furniture for this. A **Social Set** remembers who you post to and what you had written. A **signature** remembers how you sign off. A **tag** paints the calendar so a whole month reads at a glance.

## Social Sets

A Social Set is a saved composer. It holds the channels you ticked, the settings you chose on each one, and the content that was in the editor when you pressed save. The more menu and the Settings tab both say **Social Sets**.

### Save one

Open **Settings** and choose the **Social Sets** tab. The tab is deep-linkable, so `/settings?tab=sets` takes you straight there.
...
The tab heading counts them for you, as in **Social Sets (3)**, and the newest sits at the top. **Edit** reopens the composer with everything loaded and the old name waiting in the name field, so saving over it updates that set in place instead of making a second one. **Delete** asks first, by name.
...
<Note>
Tags travel over the API too. The create body carries a top-level `tags` array of `{ "value": "Launch", "label": "Launch" }` objects, and she matches the `label` against tags that already exist in your organization. A name she does not recognize is skipped rather than created, so make the tag in the app first. A soft-deleted tag can still attach by `label` over the API. Rename is `PUT /posts/tags/:id`; the picker has no rename. Full split: [Tags and hashtags](/using/tags-and-hashtags).
</Note>
...
|---|---|---|
| Social Sets | Settings, **Social Sets** tab | The tab is not listed |
...
Self-hosting? With billing switched off, your account reports itself as AGENCY, the tier labelled Ultimate, and both tabs are in the left rail of Settings from the first login. The full rule is in [Plans and limits](/using/plans-and-limits).
