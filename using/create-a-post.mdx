...
- Once the post has gone out, it opens with **Duplicate Post** only, and the repeat menu is gone. To stop a repeat that is running, delete the post with **Delete Post** on the calendar, on each channel it repeats on. Copies already published stay on the network.
...
- PostQueen does not shorten links today. Links are published as you wrote them, whatever **Settings > Global Settings > Shortlink Settings** is set to.
