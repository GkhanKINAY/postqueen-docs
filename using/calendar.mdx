...
{/* OVERHAUL-TODO(B1): write it from _templates/guide-task.mdx. Sources: this file (rewrite; WIP: better, use as base; audit: guide), main:using/posts/actions-and-stats.mdx (merge; WIP: better, use as base; audit: guide). Row in page-status-2026-09-23.md. */}
...
<AccordionGroup>
  <Accordion title="Day" icon="calendar-days">
  ...
  <Accordion title="Week" icon="calendar-range">
...
Click the body of the card and the editor opens on that post. Hover the strip and the tag names give way to a row of icons. Preview, duplicate, statistics, delete and the client share link: [Post actions and stats](/using/calendar).
...
<Note>
Tags paint the calendar. They do not change what it shows: the calendar filters on dates and customers, and the list adds state on top of that. Tags versus `#hashtags`: [Tags and hashtags](/using/sets-signatures-and-tags).
</Note>
...
| Anywhere, if you keep [sets](/using/sets-signatures-and-tags) | Asks **Select a Set** first, and **Continue without set** is always one of the answers |
...
<CardGroup cols={2}>
  ...
  <Card title="Tags and saved sets" icon="refresh-cw" href="/using/sets-signatures-and-tags">
