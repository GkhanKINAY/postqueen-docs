...
<Info>
Every network is listed in **Add Channel** on PostQueen, but some are marked **Soon**: they cannot be connected yet, and a few are **In review** with a limit until the platform approves PostQueen's app. The [status of every network](/providers/overview#status-on-postqueen) says which. Running your own instance? Some networks only appear after you register an OAuth app and restart. Each [self-hosting provider page](/installation/providers/instagram) walks through that.
</Info>
